...
const ASLEEP_VALUES = ['ASLEEP', 'ASLEEP_CORE', 'ASLEEP_DEEP', 'ASLEEP_REM', 'INBED', 3];
...
export interface MeasuredSleepWindow { start: string; end: string; nights: number; }
 ...
 */
export async function getRecentSleepWindow(days = 7): Promise<MeasuredSleepWindow | null> {
      ...
      AppleHealthKit.getSleepSamples(
        ...
        (err: any, results: any[]) => {
          ...
          for (const s of asleep) {
            ...
            const key = new Date(onset.getTime() - 12 * 3_600_000).toISOString().slice(0, 10);
            const g = nights.get(key);
            if (!g) { nights.set(key, { onset, wake }); }
            else {
      ...
      AppleHealthKit.getHeartRateSamples(
        ...
        (err: any, results: any[]) => {
          ...
          for (const r of results) {
            if (typeof r.value === 'number' && r.value < min) { min = r.value; at = new Date(r.startDate); }
          }
          resolve(at ? `${String(at.getHours()).padStart(2, '0')}:${String(at.getMinutes()).padStart(2, '0')}` : null);
        },
      );
    } catch { resolve(null); }
  });

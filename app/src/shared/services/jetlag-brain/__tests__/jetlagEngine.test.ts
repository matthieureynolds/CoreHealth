...
describe('strategy selection', () => {
  it('long eastward stay → full adjustment', () => {
    const e = buildEnhancedTrip(trip({ returnDate: addDays(DEP, 14) }), 'outbound', SLEEP);
    expect(EnhancedJetLagService.getPlanSummary(e).mode).toBe('full');
  });

  it('short eastward stay → anchor (stay on home time)', () => {
    const e = buildEnhancedTrip(trip({ returnDate: addDays(DEP, 3) }), 'outbound', SLEEP);
  ...
  it('medium eastward stay → partial adjustment', () => {
    const e = buildEnhancedTrip(trip({ returnDate: addDays(DEP, 6) }), 'outbound', SLEEP);
    expect(EnhancedJetLagService.getPlanSummary(e).mode).toBe('partial');
  });

  it('small shift → minimal', () => {
    const e = buildEnhancedTrip(trip({ destination: 'Paris', timezone: 'Europe/Paris' }), 'outbound', SLEEP);
  ...
  it('return leg only undoes the adaptation achieved outbound', () => {
    ...
    const ret = buildEnhancedTrip(trip({ returnDate: addDays(DEP, 3) }), 'return', SLEEP, undefined, undefined, undefined, 0);
...
describe('plan generation', () => {
  ...
  it('honours melatonin/caffeine prefs', () => {
    const prefs = { chronotype: 'neutral' as const, planStyle: 'gentle' as const, caffeine: false, melatonin: false, naps: false };
    const e = buildEnhancedTrip(trip({ returnDate: addDays(DEP, 14) }), 'outbound', SLEEP, prefs);
    const days = EnhancedJetLagService.generatePlan(e);
  ...
  it('flags a commitment that lands in the circadian low', () => {
    ...
    const e = buildEnhancedTrip(
      trip({
        ...
        commitments: [{ title: 'Board meeting', date_local: arrivalDate, start_local: '06:00', end_local: '07:00' }],
      }),
      'outbound',
      SLEEP,
    );
    const days = EnhancedJetLagService.generatePlan(e);
    const commit = days.flatMap(d => d.actions).find(a => a.type === 'commitment');
  ...
  it('learns direction from measured progress (observed vs expected rate)', () => {
    ...
    const onTrack = buildEnhancedTrip(trip({ returnDate: ret }), 'outbound', SLEEP, undefined, undefined, undefined, undefined,
    ...
    const behind = buildEnhancedTrip(trip({ returnDate: ret }), 'outbound', SLEEP, undefined, undefined, undefined, undefined,
      { measuredNow: { day_offset: 3, cbt_min_local: '14:00' } });
    expect(EnhancedJetLagService.getObservedShiftRate(behind)!.observedPerDay).toBeLessThan(0.3);
  });

  it('personalised efficiency flips the advance/delay crossover', () => {
    const m = { measuredNow: { day_offset: 2, cbt_min_local: '12:00' } };
    const def = buildEnhancedTrip(trip({}), 'outbound', SLEEP, undefined, undefined, undefined, undefined, m);
    expect(EnhancedJetLagService.getObservedShiftRate(def)!.direction).toBe('advance');
    // A poor advancer (0.6×) → advancing costs more days → flips to delay.
    const poorAdv = buildEnhancedTrip(trip({}), 'outbound', SLEEP, undefined, undefined, undefined, undefined,
      { ...m, advanceEfficiency: 0.6 });
    expect(EnhancedJetLagService.getObservedShiftRate(poorAdv)!.direction).toBe('delay');
  });
});

...
describe("computeAdaptationFactor", () => {
  ...
  it("slows further with chronic sleep debt", () => {
    const rested = computeAdaptationFactor({
      age: 40,
      measuredSleep: { start: "23:00", end: "07:00" },
    });
    const deprived = computeAdaptationFactor({
      age: 40,
      measuredSleep: { start: "01:00", end: "05:00" },
    });
  ...
  it("respects light sensitivity and clamps to [0.6, 1.1]", () => {
    expect(
      computeAdaptationFactor({ age: 30, lightSensitivity: "high" }),
    ).toBeLessThanOrEqual(1.1);
    expect(
      computeAdaptationFactor({ age: 90, lightSensitivity: "low" }),
...
describe("updateDirectionalEfficiency", () => {
  ...
  it("raises efficiency when faster, clamped to ≤1.5", () => {
    ...
    for (let i = 0; i < 20; i++)
      eff = updateDirectionalEfficiency(eff, "delay", 3, 1);
  ...
  it("ignores nonsensical inputs", () => {
    expect(
      updateDirectionalEfficiency(DEFAULT_EFFICIENCY, "advance", 1, 0),
    ).toEqual(DEFAULT_EFFICIENCY);
  });
});

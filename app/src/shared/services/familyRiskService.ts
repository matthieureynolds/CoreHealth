...
export function deriveFeaturesFromSignals(signals: HereditarySignal[]): DerivedRiskFeature[] {
  ...
  for (const s of signals) {
    ...
    if (/prostate[_\s]?cancer/i.test(s.conditionCode)) {
      if ((rel === 'parent' || rel === 'sibling') && isEarlyOnset(s.onsetAgeBand)) {
    ...
    if (/cad|coronary|myocardial|heart\s*attack|ischemic/i.test(s.conditionCode)) {

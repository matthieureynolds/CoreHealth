...
export function formatHealthDataForPrompt(healthData?: HealthData): string {
  ...
  if (healthData.profile) {
    ...
    if (healthData.profile.gender)
      out += `Gender: ${healthData.profile.gender}\n`;
    if (healthData.profile.height && healthData.profile.weight) {
      const bmi = (
        healthData.profile.weight / Math.pow(healthData.profile.height / 100, 2)
      ).toFixed(1);
  ...
  // Connected Devices
  try {
    const events = Array.isArray(healthData.deviceData)
      ? healthData.deviceData
      : [];
    const types = Array.from(
      new Set(events.map((d: any) => d?.deviceType)),
    ).filter(Boolean);
    ...
    if (times.length > 0)
      out += `Last Device Sync: ${new Date(Math.max(...times)).toISOString()}\n`;
  } catch {
    /* graceful degradation */
  }
  ...
  try {
    ...
    if (s) {
      ...
      if (generalParts.length)
        out += `Display & Format: ${generalParts.join(", ")}\n`;

      const accParts: string[] = [];
      if (accessibility.fontSize)
        accParts.push(`Font: ${accessibility.fontSize}`);
      if (accessibility.highContrast !== undefined)
        accParts.push(
          `HighContrast: ${accessibility.highContrast ? "on" : "off"}`,
        );
      if (accessibility.reducedMotion !== undefined)
        accParts.push(
          `ReducedMotion: ${accessibility.reducedMotion ? "on" : "off"}`,
        );
      if (accessibility.hapticFeedback !== undefined)
        accParts.push(
          `Haptics: ${accessibility.hapticFeedback ? "on" : "off"}`,
        );
      if (accParts.length) out += `Accessibility: ${accParts.join(", ")}\n`;

      const privParts: string[] = [];
      if (privacy.biometricAuth !== undefined)
        privParts.push(`Biometric: ${privacy.biometricAuth ? "on" : "off"}`);
      if (privacy.twoFactorAuth !== undefined)
        privParts.push(`2FA: ${privacy.twoFactorAuth ? "on" : "off"}`);
      if (privacy.sessionTimeout)
        privParts.push(`Timeout: ${privacy.sessionTimeout}`);
      if (privacy.locationServices !== undefined)
        privParts.push(`Location: ${privacy.locationServices ? "on" : "off"}`);
      if (privacy.dataSharing) {
        const ds = privacy.dataSharing;
        privParts.push(
          `Sharing: analytics=${ds.analytics ? "on" : "off"}, anonymized=${ds.anonymizedData ? "on" : "off"}, thirdParty=${ds.thirdPartyApps ? "on" : "off"}`,
        );
      }
      if (privParts.length)
        out += `Privacy & Security: ${privParts.join(", ")}\n`;

      const syncParts: string[] = [];
      if (dataSync.autoSync !== undefined)
        syncParts.push(`AutoSync: ${dataSync.autoSync ? "on" : "off"}`);
      if (dataSync.backgroundSync !== undefined)
        syncParts.push(`Background: ${dataSync.backgroundSync ? "on" : "off"}`);
      if (dataSync.syncFrequency)
        syncParts.push(`Frequency: ${dataSync.syncFrequency}`);
      if (dataSync.wifiOnly !== undefined)
        syncParts.push(`WiFiOnly: ${dataSync.wifiOnly ? "on" : "off"}`);
      if (dataSync.dataRetention)
        syncParts.push(`Retention: ${dataSync.dataRetention}`);
      if (dataSync.backupEnabled !== undefined)
        syncParts.push(`Backup: ${dataSync.backupEnabled ? "on" : "off"}`);
      if (dataSync.backupFrequency)
        syncParts.push(`BackupEvery: ${dataSync.backupFrequency}`);
      if (syncParts.length) out += `Data & Sync: ${syncParts.join(", ")}\n`;

      const notifParts: string[] = [];
      if (notifications.enabled !== undefined)
        notifParts.push(`Enabled: ${notifications.enabled ? "on" : "off"}`);
      [
        "healthSummaries",
        "biomarkerAlerts",
        ...
        if (notifications[k] !== undefined)
          notifParts.push(`${k}:${notifications[k] ? "on" : "off"}`);
      });
      if (notifications.quietHours?.enabled) {
        notifParts.push(
          `QuietHours: ${notifications.quietHours.startTime}-${notifications.quietHours.endTime}`,
        );
      }
      ...
      if (biomarkers.displaySettings) {
        ...
        const bds: string[] = [];
        if (ds.showTrends !== undefined)
          bds.push(`Trends:${ds.showTrends ? "on" : "off"}`);
        if (ds.showPercentiles !== undefined)
          bds.push(`Percentiles:${ds.showPercentiles ? "on" : "off"}`);
        if (ds.groupByCategory !== undefined)
          bds.push(`GroupBy:${ds.groupByCategory ? "category" : "none"}`);
  ...
  try {
    ...
    if (systems.length) {
      const rows = systems
        ...
        .map(
          (bs: any) =>
            `${bs.name || bs.id}: ${typeof bs.riskScore === "number" ? bs.riskScore : "-"}`,
        )
        .join("; ");
  ...
  try {
    ...
    if (th) {
      ...
      if (th.airQuality?.value !== undefined)
        parts.push(`AQI:${th.airQuality.value}`);
      if (th.pollenLevels?.value !== undefined)
        parts.push(`Pollen:${th.pollenLevels.value}`);
  ...
  // Recent Lab Results
  try {
    const labs = Array.isArray(healthData.labResults)
...
function assessBiomarkerStatus(biomarker: Biomarker): string {
  ...
  const ranges: Record<
    string,
    { optimal: [number, number]; normal: [number, number] }
  > = {

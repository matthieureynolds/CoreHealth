...
export const OPENAI_API_URL_INSIGHTS = 'https://api.openai.com/v1/chat/completions';

export function parseInsightsResponse(content: string): HealthAssistantResponse {
...
Keep it conversational and helpful, not overly clinical.`;
    ...
    const response = await fetch(OPENAI_API_URL_INSIGHTS, {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: 'You are a friendly health assistant providing insights from health data.' },
...
Make them actionable, friendly, and relevant to their health situation. Focus on simple things they can do today.`;
    ...
    const response = await fetch(OPENAI_API_URL_INSIGHTS, {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: 'You are a friendly health assistant providing daily recommendations.' },
          { role: 'user', content: prompt }
        ],
        temperature: 0.7,
        max_tokens: 400,

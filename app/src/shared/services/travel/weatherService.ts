...
export const generateExtremeHeatWarning = (
  ...
  const uvHeatCombination = calculateUVHeatCombination(
    uvIndex,
    temp,
    heatIndexData.heatIndex,
  );
...
export const calculateUVHeatCombination = (
  ...
  // UV contribution
  if (uvIndex >= 11)
    riskScore += 4; // Extreme UV
  else if (uvIndex >= 8)
    riskScore += 3; // Very High UV
  else if (uvIndex >= 6)
    riskScore += 2; // High UV
  ...
  // Heat contribution
  if (heatIndex >= 45)
    riskScore += 4; // Extreme heat
  else if (heatIndex >= 40)
    riskScore += 3; // Very high heat
  else if (heatIndex >= 35)
    riskScore += 2; // High heat
...
export const generateWeatherHealthAssessment = async (
  ...
  try {
    ...
    const extremeHeatWarning = generateExtremeHeatWarning(
      weatherData,
      uvIndex,
      timeOfDay,
    );

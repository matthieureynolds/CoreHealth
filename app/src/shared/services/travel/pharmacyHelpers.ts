...
export const processOpeningHours = (openingHours: any): MedicationPharmacy['openingHours'] => {
  if (!openingHours) {
      ...
      if (period) {
        if (i === 0 && currentStatus === 'open' && period.close) {
          ...
          if (parseInt(closeTime) > currentTime) {
            nextOpenClose = { time: formatTime(closeTime), day: i === 0 ? 'today' : getDayName(checkDay) };
            break;
          }
        } else if (currentStatus === 'closed' && period.open) {
          const openTime = period.open.time;
          if (i > 0 || parseInt(openTime) > currentTime) {
            nextOpenClose = { time: formatTime(openTime), day: i === 0 ? 'today' : getDayName(checkDay) };
            break;
          }
        }
      }
  ...
  return { weekdayText: openingHours.weekday_text || [], currentStatus, nextOpenClose };
};

export const determinePharmacyType = (types: string[], name: string): MedicationPharmacy['pharmacyType'] => {
...
export const determinePharmacyServices = (types: string[], name: string): string[] => {
  const services: string[] = [];
  const nameUpper = name.toUpperCase();
  if (types.includes('pharmacy')) services.push('Prescription filling', 'Over-the-counter medications');
  if (types.includes('health') || nameUpper.includes('HEALTH')) services.push('Health screenings', 'Vaccinations');
  if (nameUpper.includes('24') || nameUpper.includes('HOUR')) services.push('24-hour service');
  if (nameUpper.includes('COMPOUND')) services.push('Compounding pharmacy');
  if (nameUpper.includes('DRIVE') || nameUpper.includes('THRU')) services.push('Drive-through service');
  if (['CVS', 'WALGREENS'].some(chain => nameUpper.includes(chain))) services.push('Photo services', 'MinuteClinic', 'Insurance accepted');
  return services;
};

export const determineAccessibility = (types: string[]): string[] => {
  const accessibility = ['Wheelchair accessible entrance'];
  if (types.includes('hospital') || types.includes('health')) accessibility.push('Handicapped parking', 'Accessible restrooms');
  return accessibility;
};

export const determinePaymentMethods = (pharmacyType: MedicationPharmacy['pharmacyType']): string[] => {
  const methods = ['Cash', 'Credit cards', 'Debit cards'];
  if (pharmacyType === 'chain' || pharmacyType === 'supermarket') methods.push('Insurance plans', 'FSA/HSA cards', 'Mobile payments');
  else if (pharmacyType === 'hospital') methods.push('Insurance plans', 'Hospital billing');
  else methods.push('Most insurance plans');
  return methods;
};

export const determineLanguages = (name: string, pharmacyType: MedicationPharmacy['pharmacyType']): string[] => {
  const languages = ['English'];
  if (pharmacyType === 'chain') languages.push('Spanish', 'Multilingual staff available');
  if (pharmacyType === 'hospital') languages.push('Translator services available');
  return languages;
};

export const determinePharmacySpecialties = (types: string[], name: string): string[] => {
  const specialties: string[] = [];
  if (types.includes('hospital')) specialties.push('Hospital pharmacy', 'Controlled substances', 'Prescription medications');
  if (types.includes('pharmacy')) specialties.push('Prescription medications', 'Over-the-counter drugs');
  if (name.toLowerCase().includes('24') || name.toLowerCase().includes('hour')) specialties.push('24-hour service');
  if (name.toLowerCase().includes('compounding')) specialties.push('Compounding pharmacy');
  return specialties;
};

export const removeDuplicatePharmacies = (pharmacies: MedicationPharmacy[]): MedicationPharmacy[] => {
  const uniquePharmacies: MedicationPharmacy[] = [];
  for (const pharmacy of pharmacies) {
    const isDuplicate = uniquePharmacies.some(existing =>
      calculateDistance(pharmacy.coordinates.latitude, pharmacy.coordinates.longitude, existing.coordinates.latitude, existing.coordinates.longitude) < 100
    );
...
export const findNearbyPharmacies = async (
  ...
  try {
    ...
    const searchTypes = medicationType === 'controlled_substance' ? ['hospital', 'pharmacy'] : ['pharmacy'];
    ...
    for (const type of searchTypes) {
      ...
      for (const place of data.results || []) {
        if (place.place_id) {
          const detailedPharmacy = await getDetailedPharmacyInfo(place.place_id, medicationType, latitude, longitude);

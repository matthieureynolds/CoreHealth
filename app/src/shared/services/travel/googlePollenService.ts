 ...
 */
export const getOverallPollenRiskLevel = (
  pollenData: GooglePollenData,
 ...
 */
export const getPollenRecommendations = (
  pollenData: GooglePollenData,
): string => {
  const { tree, grass, weed } = pollenData.pollen;
  const maxIndex = Math.max(tree.indexValue, grass.indexValue, weed.indexValue);
 ...
 */
export const getPollenHealthRecommendations = (
  pollenData: GooglePollenData,
): string[] => {
  ...
  if (maxIndex > 2) {
    recommendations.push(
      "Limit outdoor activities during peak pollen hours (5-10 AM)",
    );

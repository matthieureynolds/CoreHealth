...
import { clearCorruptedHealthScore } from "./healthDataLoader";
...
export const HealthDataProvider: React.FC<HealthDataProviderProps> = ({
  ...
  const [jetLagPlanningEvents, setJetLagPlanningEvents] = useState<
    JetLagPlanningEvent[]
  >([]);
  const [isLoading, setIsLoading] = useState(true);
  const [originTimezone, setOriginTimezoneState] = useState<string | null>(
    null,
  );
  const [originLocation, setOriginLocationState] = useState<string>("Home");
  const [derivedRiskFeatures, setDerivedRiskFeatures] = useState<
    DerivedRiskFeature[]
  >([]);
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("@corehealth_location_consent")
      .then((consented) => {
        ...
        fetchAuthSession()
          .then((session) => {
            ...
            if (userId)
              api
                .put(`/users/${userId}/location-health`, payload)
                .catch(() => {});
          })
          .catch(() => {});
      })
  ...
  const recalculateHealthScore = useCallback(
    async (currentBiomarkers: Biomarker[]) => {
      await _recalculateHealthScore(
        currentBiomarkers,
      ...
      try {
        ...
        if (user?.id) {
          await DataService.updateProfile(user.id, {
            firstName:
              (profileUpdates as any).firstName ??
              (updatedProfile as any).firstName,
            surname:
              (profileUpdates as any).surname ??
              (updatedProfile as any).surname,
  ...
  const updateBiomarker = useCallback(
    async (id: string, updates: Partial<Biomarker>) => {
      try {
  ...
  const updateTravelHealthData = useCallback(
  ...
  const setOriginTimezone = useCallback(
    async (timezone: string, location?: string) => {
      try {
        ...
        if (travelHealth && travelHealth.coordinates) {
          const locationData: LocationData = {
            ...
            elevation:
              typeof travelHealth.altitudeRisk.value === "number"
                ? travelHealth.altitudeRisk.value
                : 0,
          };
  ...
  const calculateJetLag = useCallback(
    (
      destinationTimezone: string,
      destinationLocation: string,
    ): JetLagData | null => {
  ...
  const addJetLagPlanningEvent = useCallback(
    async (
      event: Omit<JetLagPlanningEvent, "id" | "createdAt" | "updatedAt">,
    ) => {
      try {
        await _addJetLagPlanningEvent(
          event,
  ...
  const updateJetLagPlanningEvent = useCallback(
    async (id: string, updates: Partial<JetLagPlanningEvent>) => {
      try {
        await _updateJetLagPlanningEvent(
          id,
          updates,
          jetLagPlanningEvents,
          setJetLagPlanningEvents,
  ...
  const deleteJetLagPlanningEvent = useCallback(
    async (id: string) => {
      try {
        await _deleteJetLagPlanningEvent(
          id,
          jetLagPlanningEvents,
          setJetLagPlanningEvents,
        );

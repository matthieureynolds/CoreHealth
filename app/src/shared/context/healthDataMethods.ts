...
const BODY_SYSTEM_DEFS: Array<{
  ...
  {
    id: 'cardiovascular', name: 'Cardiovascular', coordinates: { x: 50, y: 30 },
    biomarkerCategories: ['cardiovascular'],
    imagingModalities: ['ecg', 'echo', 'x-ray'],
    impressionKeywords: /cardiac|heart|coronary|arrhythmia|cardiomegaly|atrial|ventricular|aorta|valve/i,
  },
  {
    id: 'metabolic', name: 'Metabolic', coordinates: { x: 50, y: 45 },
    biomarkerCategories: ['metabolic'],
    imagingModalities: ['ultrasound', 'ct'],
    impressionKeywords: /liver|pancreas|glucose|insulin|thyroid|metabolic|fatty|hepat/i,
  },
  {
    id: 'hormonal', name: 'Hormonal', coordinates: { x: 60, y: 40 },
    biomarkerCategories: ['hormonal'],
    imagingModalities: ['ultrasound', 'mri'],
    impressionKeywords: /thyroid|adrenal|pituitary|ovarian|testicular|hormone|endocrine/i,
  },
  {
    id: 'inflammatory', name: 'Inflammatory', coordinates: { x: 40, y: 35 },
    biomarkerCategories: ['inflammatory'],
    imagingModalities: [],
    impressionKeywords: /inflammation|autoimmune|arthritis|synovitis/i,
  },
  {
    id: 'nutritional', name: 'Nutritional', coordinates: { x: 55, y: 50 },
  ...
  {
    id: 'neurological', name: 'Neurological', coordinates: { x: 50, y: 10 },
    biomarkerCategories: [],
    imagingModalities: ['mri', 'ct', 'pet'],
    impressionKeywords: /brain|neuro|cognitive|cerebral|cortex|white matter|lesion|stroke/i,
  },
  {
    id: 'respiratory', name: 'Respiratory', coordinates: { x: 50, y: 25 },
    biomarkerCategories: [],
    imagingModalities: ['x-ray', 'ct'],
    impressionKeywords: /lung|pulmonary|bronch|pneumon|pleural|effusion|airway/i,
  },
  {
    id: 'musculoskeletal', name: 'Musculoskeletal', coordinates: { x: 50, y: 60 },
    biomarkerCategories: [],
    imagingModalities: ['x-ray', 'mri', 'dexa'],
    impressionKeywords: /bone|joint|fracture|osteoporosis|tendon|ligament|cartilage|spine|disc/i,
  },
];

const CONCERNING = /abnormal|severe|significant|stenosis|cardiomegaly|arrhythmia|osteoporosis|effusion|fracture|mass|lesion|malignant|critical/i;
...
function buildBodySystemsFromData(biomarkers: Biomarker[], imagingResults: any[]): BodySystem[] {
  return BODY_SYSTEM_DEFS.map(def => {
    const systemBiomarkers = biomarkers.filter(b => def.biomarkerCategories.includes(b.category));
    const abnormalCount = systemBiomarkers.filter(b => b.riskLevel === 'high' || b.riskLevel === 'medium').length;

    const biomarkerRisk = systemBiomarkers.length > 0
...
export const loadHealthData = async (
  ...
  try {
    ...
    if (userId) {
      const [biomarkers, allergies, profileRow, medications, conditions, vaccinations, imagingResults] = await Promise.all([
      ...
      const age = dbProfile.date_of_birth
        ? Math.floor((Date.now() - new Date(dbProfile.date_of_birth).getTime()) / (365.25 * 24 * 60 * 60 * 1000))
        : DEFAULT_PROFILE.age;
      ...
      setters.setProfile({
        ...
        userId,
        age,
        gender: (dbProfile.gender ?? DEFAULT_PROFILE.gender) as UserProfile['gender'],
      ...
      if (biomarkers.length === 0 && imagingResults.length === 0) {
        ...
        // ── Health score ────────────────────────────────────────────────────────
        const normalCount = biomarkers.filter(b => b.riskLevel === 'low').length;
        const biomarkerScore = biomarkers.length > 0
          ? Math.round((normalCount / biomarkers.length) * 100)
          : 80;
        const clampedBiomarkerScore = Math.max(40, Math.min(100, biomarkerScore));

        // Imaging penalty: each concerning finding reduces the score (max −20)
        const SIGNIFICANT = /abnormal|osteoporosis|stenosis|arrhythmia|cardiomegaly|effusion|fracture|mass|lesion|malignant|severe|significant/i;
        const MILD_CONCERN = /mild|borderline|minimal|slight/i;
        const imagingPenalty = Math.min(20, imagingResults.reduce((pen: number, img: any) => {
          const text = img.impression || img.findings || '';
        ...
        // ── Body systems ────────────────────────────────────────────────────────
        setters.setBodySystems(buildBodySystemsFromData(biomarkers, imagingResults));
      }

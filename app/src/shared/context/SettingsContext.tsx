...
interface SettingsContextType {
  ...
  updateLifestyleSettings: (
    updates: Partial<LifestyleSettings>,
  ) => Promise<void>;
  updateBiomarkerSettings: (
    updates: Partial<BiomarkerSettings>,
  ) => Promise<void>;
...
export const SettingsProvider: React.FC<SettingsProviderProps> = ({
    ...
    try {
      ...
      fetchAuthSession()
        .then((session) => {
          ...
          if (userId)
            api
              .put(`/users/${userId}/settings`, newSettings as any)
              .catch(() => {});
        })
        .catch(() => {});
      try {
  ...
  const applySettings = async (currentSettings: UserSettings) => {
    if (currentSettings.general.theme !== "auto") {
      Appearance.setColorScheme(
        currentSettings.general.theme as ColorSchemeName,
      );
    }
  ...
  const updateDataSyncSettings = async (updates: Partial<DataSyncSettings>) => {
    const newSettings = {
      ...settings,
      dataSync: { ...settings.dataSync, ...updates },
    };
    dispatch({ type: "UPDATE_DATA_SYNC", payload: updates });
    await saveSettings(newSettings);
  };

  const updateHealthEmergencySettings = async (
    updates: Partial<HealthEmergencySettings>,
  ) => {
    const newSettings = {
      ...settings,
      healthEmergency: { ...settings.healthEmergency, ...updates },
    };
  ...
  const updateTravelSettings = async (updates: Partial<TravelSettings>) => {
    const newSettings = {
      ...settings,
      travel: { ...settings.travel, ...updates },
    };
  ...
  const updateLifestyleSettings = async (
    updates: Partial<LifestyleSettings>,
  ) => {
    const newSettings = {
      ...settings,
      lifestyle: { ...settings.lifestyle, ...updates },
    };
    dispatch({ type: "UPDATE_LIFESTYLE", payload: updates });
    await saveSettings(newSettings);
  };

  const updateBiomarkerSettings = async (
    updates: Partial<BiomarkerSettings>,
  ) => {
    const newSettings = {
      ...settings,
      biomarkers: { ...settings.biomarkers, ...updates },
    };
  ...
  const updateSettings = async (category: keyof UserSettings, updates: any) => {
    ...
    dispatch({
      type: `UPDATE_${category.toUpperCase()}` as any,
      payload: updates,
    });
    await saveSettings(newSettings);
  };

import { UserSettings, SettingsAction, defaultSettings } from '../types/settings';

export function settingsReducer(state: UserSettings, action: SettingsAction): UserSettings {
  switch (action.type) {
    case 'UPDATE_GENERAL':
    ...
    case 'UPDATE_HEALTH_EMERGENCY':
      return { ...state, healthEmergency: { ...state.healthEmergency, ...action.payload } };
    case 'UPDATE_TRAVEL':
...
export function mergeWithDefaults(imported: any, defaults: UserSettings): UserSettings {

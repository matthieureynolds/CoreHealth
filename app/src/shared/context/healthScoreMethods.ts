...
function scoreBiomarkersRing(biomarkers: Biomarker[]): number {
  ...
  if (glucose !== undefined)
    metabolic.push({ score: scoreGlucose(glucose), weight: 0.2 });
  const hba1c = find(/hba1c|a1c/i);
  if (hba1c !== undefined)
    metabolic.push({ score: scoreHbA1c(hba1c), weight: 0.25 });
  const homaIR = find(/homa.?ir/i);
  if (homaIR !== undefined)
    metabolic.push({ score: scoreHomaIR(homaIR), weight: 0.25 });
  const trig = find(/triglyceride/i);
  if (trig !== undefined)
    metabolic.push({ score: scoreTriglycerides(trig), weight: 0.15 });
  ...
  if (apob !== undefined) cardio.push({ score: scoreApoB(apob), weight: 0.4 });
  const hsCRP = find(/hs.?crp/i);
  if (hsCRP !== undefined)
    cardio.push({ score: scoreHsCRP(hsCRP), weight: 0.25 });
  const ldl = find(/^ldl/i);
  if (ldl !== undefined) cardio.push({ score: scoreLDL(ldl), weight: 0.2 });
  const totalChol = find(/total chol/i);
  if (totalChol !== undefined)
    cardio.push({ score: scoreTotalCholesterol(totalChol), weight: 0.15 });

  // Inflammation cluster (15% of biomarkers ring)
  const inflam: WeightedScore[] = [];
  if (hsCRP !== undefined)
    inflam.push({ score: scoreHsCRP(hsCRP), weight: 0.6 });
  const homocys = find(/homocysteine/i);
  if (homocys !== undefined)
    inflam.push({ score: scoreHomocysteine(homocys), weight: 0.4 });
  ...
  if (metabolicScore !== null)
    clusters.push({ score: metabolicScore, weight: 0.35 });
...
function scoreRecoveryRing(
  ...
  if (hrv !== undefined) items.push({ score: scoreHRV(hrv), weight: 0.3 });
  ...
  if (rhr !== undefined)
    items.push({ score: scoreRestingHR(rhr), weight: 0.1 });
...
export const recalculateHealthScore = async (
  ...
  const raw = Math.round(
    biomarkersRing * 0.45 + recoveryRing * 0.35 + lifestyleRing * 0.2,
  );
...
function buildBiomarkerInsights(
  biomarkers: Biomarker[],
  ...
  if (cardio.length > 0) {
    ...
    insights.push({
      ...
      action: allGood
        ? undefined
        : "Review your cholesterol and CRP trends in the Body Map.",
    });
  }
  ...
  if (healthScore && insights.length === 0) {
    ...
    insights.push({
      ...
      actionable: score < 80,
      action:
        score < 80 ? "Open Toto for personalised recommendations." : undefined,
    });
  }
  ...
  insights.push({
    ...
    description:
      "Even 20 minutes of moderate activity improves cardiovascular health, HRV, and cognitive performance.",

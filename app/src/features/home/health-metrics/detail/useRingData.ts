...
function bucketByDay(
  ...
  readings.forEach(({ value, date }) => {
    const dayOffset = Math.floor(
      (date.getTime() - monday.getTime()) / 86_400_000,
    );
...
function bucketByWeek(
  ...
  readings.forEach(({ value, date }) => {
    const dayOfMonth = Math.floor(
      (date.getTime() - monthStart.getTime()) / 86_400_000,
    );
...
function buildLiveConfigs(
  ...
  (["recovery", "biomarkers", "lifestyle"] as RingId[]).forEach((ringId) => {
    ...
    const liveStats = mock.stats.map((stat): StatDef => {
      ...
      const allReadings = [...readings, ...deviceReadings].sort(
    ...
    const liveDay = anyLive
      ? deriveScores(liveStats, "day", mock.dayScores)
      : mock.dayScores;
    const liveWeek = anyLive
      ? deriveScores(liveStats, "week", mock.weeklyScores)
      : mock.weeklyScores;
    const liveMonth = anyLive
      ? deriveScores(liveStats, "month", mock.monthScores)
      : mock.monthScores;
    const liveYear = anyLive
      ? deriveScores(liveStats, "year", mock.yearScores)
      : mock.yearScores;
...
function extractDeviceReadings(
  statId: string,
  deviceData: any[],
  ...
  deviceData.forEach((d) => {
    ...
    switch (statId) {
      case "hrv":
        val = extract(["hrv", "heartRateVariability", "rmssd"]);
        break;
      case "rhr":
        val = extract(["restingHeartRate", "resting_hr", "rhr"]);
        break;
      case "sleep_duration":
        val = extract(["sleepHours", "sleep_hours", "totalSleep"]);
        break;
      case "deep_sleep":
        val = extract(["deepSleepHours", "deep_sleep_hours"]);
        break;
      case "sleep_consistency":
        val = extract(["sleepConsistency", "sleep_consistency"]);
        break;
      case "vo2_max":
        val = extract(["vo2Max", "vo2_max"]);
        break;
    }

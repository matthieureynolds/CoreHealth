...
const HeroRing: React.FC<{ score: number; color: string }> = ({
  score,
  color,
}) => {
  const size = 160;
...
const StatRow: React.FC<{
  ...
  const trendColor =
    trend === "up" ? "#30D158" : trend === "down" ? "#FF3B30" : "#8E8E93";
  const trendIcon =
          ...
          <View style={styles.subMetricsRow}>
            {config.subMetrics.map((m) => (
              <View key={m.label} style={styles.subMetricBox}>
                ...
                <View
                  style={[
                    styles.subMetricBadge,
                    { backgroundColor: `${m.badgeColor}22` },
                  ]}
                >
                  <Text
                    style={[styles.subMetricBadgeText, { color: m.badgeColor }]}
                  >
                    {m.badge}
                  </Text>
                </View>

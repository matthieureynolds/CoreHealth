...
const getHomeStatus = (score: number, metricId: string, config: MetricConfig): string => {
  const pos = score / config.divisor;
  if (pos <= 0.2) return ['water_quality', 'food_safety'].includes(metricId) ? 'hazardous' : 'excellent';
  if (pos <= 0.4) return ['water_quality', 'food_safety'].includes(metricId) ? 'poor' : 'good';
  if (pos <= 0.6) return 'moderate';
  if (pos <= 0.8) return ['water_quality', 'food_safety'].includes(metricId) ? 'good' : 'poor';
  return ['water_quality', 'food_safety'].includes(metricId) ? 'excellent' : 'hazardous';
};
...
const METRIC_CONFIGS: Record<string, MetricConfig> = {
  air_quality: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent': return 'Nothing to worry about';
        case 'good': return 'You\'re all clear';
        case 'moderate': return 'Be mindful if you\'re sensitive';
        case 'poor': return 'Limit your time outdoors today';
        case 'hazardous': return 'Stay indoors if you can';
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        case 'excellent':
          return ['Perfect for a run or outdoor workout', 'Open the windows and let fresh air in'];
        case 'good':
          return ['Go ahead with any outdoor plans', 'No precautions needed'];
        case 'moderate':
          return ['Keep intense outdoor workouts short', 'Close windows during peak traffic hours', 'Run an air purifier if you\'re sensitive'];
        case 'poor':
          return ['Skip the outdoor workout today', 'Keep windows shut', 'Grab an N95 if you need to go out'];
        case 'hazardous':
          return ['Stay inside — windows and doors shut', 'Run your air purifier on high', 'N95 mask is a must if you go outside'];
        default:
    ...
    getComparison: (status: string, homeStatus: string) => {
      ...
      const ranks: Record<string, number> = { excellent: 5, good: 4, moderate: 3, poor: 2, hazardous: 1 };
  ...
  pollen: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent': return 'Allergy-free vibes today';
        case 'good': return 'You should be fine outside';
        case 'moderate': return 'Pop an antihistamine before heading out';
        case 'poor': return 'Tough day for allergy sufferers';
        case 'hazardous': return 'Stay inside if you can';
        default: return 'Checking conditions';
      }
    },
    getMeaning: (status: string) => {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'good':
          return ['Enjoy the outdoors', 'Quick shower after a long time outside doesn\'t hurt'];
        case 'moderate':
          return ['Antihistamine before you go out', 'Avoid early mornings when pollen peaks', 'Shower and change when you get home'];
        case 'poor':
          return ['Take your allergy meds', 'Keep windows closed at home', 'Sunglasses help keep pollen out of your eyes'];
        case 'hazardous':
          return ['Stay inside with windows shut', 'Run an air purifier if you have one', 'Talk to your doctor if symptoms are bad'];
        default:
  ...
  water_quality: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent': return 'Drink straight from the tap';
        case 'good': return 'Tap water is safe here';
        case 'moderate': return 'Consider bottled or filtered water';
        case 'poor': return 'Don\'t drink the tap water';
        case 'hazardous': return 'Bottled water only';
        default: return 'Checking water safety';
      }
    },
    getMeaning: (status: string) => {
      switch (status) {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'good':
          return ['Tap water is perfectly safe', 'A filter can improve taste if you\'re fussy'];
        case 'moderate':
          return ['Use a filter or bottled water for drinking', 'Tap is fine for showering and washing'];
        case 'poor':
          return ['Bottled or filtered water only', 'Skip ice at restaurants', 'Use bottled water for cooking too'];
        case 'hazardous':
          return ['Sealed bottled water for everything', 'That includes brushing your teeth', 'Avoid ice and raw washed produce'];
        default:
  ...
  uv_index: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent':
        case 'good': return 'No sun worries today';
        case 'moderate': return 'Sunscreen if you\'re out long';
        case 'poor': return 'Cover up and use SPF 50+';
        case 'hazardous': return 'Avoid direct sun';
        default: return 'Checking UV levels';
      }
    },
    getMeaning: (status: string) => {
      switch (status) {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'moderate':
          return ['SPF 30+ for extended time outside', 'Hat and sunglasses are a good idea', 'Find shade around midday'];
        case 'poor':
          return ['SPF 50+ is a must', 'Hat, sunglasses, and long sleeves', 'Stay out of direct sun 10am–4pm'];
        case 'hazardous':
          return ['Avoid being outside 10am–4pm', 'SPF 50+ on all exposed skin', 'Full coverage clothing and shade'];
        default:
  ...
  food_safety: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent':
        case 'good': return 'Eat freely, just wash your hands';
        case 'moderate': return 'Be choosy about where you eat';
        case 'poor': return 'Stick to cooked food from trusted places';
        default: return 'Checking food safety';
      }
    },
    getMeaning: (status: string) => {
      switch (status) {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'moderate':
          return ['Go for freshly cooked food', 'Skip raw or undercooked meats', 'Bottled water for brushing teeth'];
        case 'poor':
          return ['Avoid street food and raw produce', 'Only drink sealed bottled water', 'Pack some rehydration salts just in case'];
        default:
  ...
  altitude: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent':
        case 'good': return 'No altitude concerns';
        case 'moderate': return 'Take it easy on day one';
        case 'poor': return 'Your body needs time to adjust';
        default: return 'Checking altitude';
      }
    },
    getMeaning: (status: string) => {
      switch (status) {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'moderate':
          return ['Ease into exercise on day one', 'Drink more water than usual', 'Skip alcohol for the first day'];
        case 'poor':
          return ['Give yourself 1-2 days to acclimatise', 'Don\'t rush the ascent', 'Talk to your doctor about acetazolamide'];
        default:
  ...
  outbreaks: {
    ...
    getTakeaway: (status: string) => {
      switch (status) {
        case 'excellent':
        case 'good': return 'No outbreaks to worry about';
        case 'moderate': return 'Some local outbreaks — stay aware';
        case 'poor': return 'Take extra precautions';
        default: return 'Checking outbreak data';
      }
    },
    getMeaning: (status: string) => {
      switch (status) {
    ...
    getRecommendations: (status: string) => {
      switch (status) {
        ...
        case 'moderate':
          return ['Wash your hands more often', 'Avoid packed indoor spaces', 'Mask up if local authorities advise it'];
        case 'poor':
          return ['Think about postponing non-essential visits', 'Strict hand hygiene and masking', 'Follow local health advisories closely'];
        default:
...
const getSegmentColor = (segments: { label: string; color: string }[], position: number): string => {
  const idx = Math.min(Math.floor(position * segments.length), segments.length - 1);
  return segments[Math.max(0, idx)].color;
};

// Derive a status key from the position + segments
const deriveStatus = (segments: { label: string; color: string }[], position: number): string => {
  ...
  return (
    <View>
      ...
      <View style={{ flexDirection: 'row', marginTop: 8 }}>
        {segments.map((seg, i) => (
          <Text key={i} style={{ flex: 1, textAlign: 'center', fontSize: 10, color: '#636366', fontWeight: '500', lineHeight: 13 }} numberOfLines={2}>
            {seg.label}
          </Text>
        ))}
      </View>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 16, gap: 6 }}>
        ...
        <Text style={{ color: comparisonColor, fontSize: 15, fontWeight: '600' }}>
          {comparisonText}
        </Text>
      </View>
    </View>
...
const EnvironmentalMetricScreen: React.FC = () => {
  ...
  const homePosition = Math.max(0, Math.min(1, config.homeScore / config.divisor));
  ...
  const comparisonRanks: Record<string, number> = { excellent: 5, good: 4, moderate: 3, poor: 2, hazardous: 1 };
  const diff = (comparisonRanks[actualStatus] || 3) - (comparisonRanks[homeStatus] || 3);
  const comparisonColor = diff > 0 ? '#30D158' : diff < 0 ? '#FF9F0A' : '#8E8E93';
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, marginBottom: 8 }}>
            <TouchableOpacity onPress={() => navigation.goBack()} style={{ padding: 6 }} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
              <Ionicons name="chevron-back" size={22} color="#FFFFFF" />
            </TouchableOpacity>
            <TouchableOpacity onPress={toggleInfo} style={{ padding: 6 }} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
              <Ionicons
          ...
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: `${statusColor}15`, alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
              <Ionicons name={icon as any} size={22} color={statusColor} />
            </View>
            <Text style={{ color: '#FFFFFF', fontSize: 24, fontWeight: '700', marginBottom: 6 }}>{label}</Text>
            <Text style={{ color: statusColor, fontSize: 15, fontWeight: '600' }}>{statusLabel}</Text>
          </View>
        </LinearGradient>

        {/* Range bar */}
        <View style={{ paddingHorizontal: 24, marginTop: 22 }}>
          <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '600', marginBottom: 16 }}>Current level</Text>
          <RangeBar
        ...
        {/* Divider */}
        <View style={{ height: StyleSheet.hairlineWidth, backgroundColor: '#2C2C2E', marginHorizontal: 24, marginTop: 24 }} />

        {/* What this means for you */}
        <View style={{ paddingHorizontal: 24, marginTop: 22 }}>
          <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '600', marginBottom: 10 }}>What this means for you</Text>
          <Text style={{ color: '#A1A1A6', fontSize: 15, lineHeight: 24 }}>{meaning}</Text>
        </View>
        ...
        {/* Recommendations */}
        <View style={{ paddingHorizontal: 24, marginTop: 22 }}>
          <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '600', marginBottom: 14 }}>What to do</Text>
          {recommendations.map((item, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12, gap: 12 }}>
              <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: statusColor, marginTop: 7, opacity: 0.6 }} />
              <Text style={{ color: '#A1A1A6', fontSize: 15, lineHeight: 22, flex: 1 }}>{item}</Text>
            </View>
          ))}
        </View>
        ...
        {showInfo && (
          <View style={{ paddingHorizontal: 24, marginTop: 24 }}>
            <View style={{ backgroundColor: '#1C1C1E', borderRadius: 14, padding: 18 }}>
              <Text style={{ color: '#FFFFFF', fontSize: 15, fontWeight: '600', marginBottom: 8 }}>What this measures</Text>
              <Text style={{ color: '#A1A1A6', fontSize: 14, lineHeight: 22 }}>{config.whatItMeasures}</Text>
            </View>
          </View>
        )}

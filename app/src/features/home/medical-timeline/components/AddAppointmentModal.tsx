...
const AddAppointmentModal: React.FC<AddAppointmentModalProps> = ({
  visible, editingEvent, onClose, onSave,
}) => {
  ...
  const [attachedDoc, setAttachedDoc]   = useState<{ uri: string; name: string; type?: string } | null>(null);
  ...
  const resetForm = () => {
    setTitle(''); setDoctor(''); setLocation(''); setDate(null);
    setNotes(''); setAttachedDoc(null);
    setShowSuggestions(false); setFilteredSuggestions([]);
  };
  ...
  const handleTitleChange = (text: string) => {
    setTitle(text);
    if (text.length > 0) {
      setFilteredSuggestions(APPOINTMENT_TYPES.filter(t => t.toLowerCase().includes(text.toLowerCase())));
...
const styles = StyleSheet.create({
  ...
  sheetContainer: { position: 'absolute', bottom: 0, left: 0, right: 0, width: '100%', zIndex: 1000, pointerEvents: 'box-none' },
  sheetContent: {
    backgroundColor: '#1C1C1E', borderTopLeftRadius: 24, borderTopRightRadius: 24, width: '100%',
    shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.25, shadowRadius: 12, elevation: 16,
  },
  ...
  header: {
    ...
    paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16, zIndex: 10,
  },

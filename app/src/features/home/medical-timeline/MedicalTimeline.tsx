...
const MedicalTimeline: React.FC = () => {
  ...
  const saveBpAndClose = () => {
    ...
    updateProfile({
      ...profile,
      screenings: [...(profile.screenings || []), newScreening],
    })
      .then(closeBpModal)
      .catch(closeBpModal);
  };
  ...
  const groupEntries = (
    Object.entries(visibleGrouped) as [GroupKey, MedicalEvent[]][]
  ...
  return (
    <View
      style={events.length === 0 ? styles.containerEmpty : styles.container}
    >
      ...
      <AddAppointmentModal
        ...
        onSave={async (event) => {
          ...
          try {
            ...
            if (
              datePart.toLowerCase() !== "today" &&
              datePart.toLowerCase() !== "tomorrow"
            ) {

...
const STRENGTHS = [
  {
    ...
    label: "HbA1c",
    value: "5.1%",
    note: "Optimal metabolic health",
  },
  {
    icon: "speedometer-outline",
    label: "Blood Pressure",
    value: "118/76",
    note: "Optimal vascular health",
  },
  {
    icon: "time-outline",
    label: "Sleep Consistency",
    value: "87%",
    note: "Strong circadian rhythm",
  },
];
...
const GAPS = [
  {
    ...
    label: "ApoB",
    value: "70 mg/dL",
    note: "Target <55 for optimal",
  },
  {
    icon: "sunny-outline",
    label: "Vitamin D",
    value: "35 ng/mL",
    note: "Target 55–80 ng/mL",
  },
  {
    icon: "heart-outline",
    label: "HRV",
    ...
    {PERIOD_PILLS.map(({ key, label }) => {
      ...
      return (
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.periodPillLabel, active && { color: ACCENT }]}>
            {label}
          </Text>
        </TouchableOpacity>
      );
...
const MiniRing: React.FC<{ score: number; selected: boolean }> = ({
  score,
  selected,
}) => {
  ...
  return (
    <Svg width={size} height={size}>
      ...
      />
      <Circle
        stroke={selected ? ACCENT : `${ACCENT}55`}
        fill="none"
        cx={size / 2}
        cy={size / 2}
        r={r}
    ...
    {scores.map((score, i) => (
      ...
      >
        ...
        <Text
          style={[
            styles.ringLabel,
            i === selected && { color: "#FFFFFF", fontWeight: "600" },
          ]}
        >
          {labels[i]}
        </Text>
...
const BioAgeCard: React.FC<{ score: number; chronoAge: number }> = ({
  score,
  chronoAge,
}) => {
    ...
    <View style={styles.bioAgeCard}>
      <View style={styles.bioAgeLeft}>
        ...
        <View
          style={[
            styles.bioAgeBadge,
            { backgroundColor: delta >= 0 ? "#30D15822" : "#FF3B3022" },
          ]}
        >
          <Text
            style={[
              styles.bioAgeBadgeText,
              { color: delta >= 0 ? "#30D158" : "#FF3B30" },
            ]}
          >
      ...
      <View style={styles.bioAgeRight}>
        ...
        <Text
          style={[styles.bioAgePace, { color: slower ? "#30D158" : "#FF3B30" }]}
        >
          {pace}×
        </Text>
...
const COMPOSITION_RINGS = [
  ...
  { id: "lifestyle", label: "Physical", color: "#FF9F0A", weight: 0.2 },
] as const;
        ...
        <View style={styles.section}>
          ...
          <View style={styles.compRow}>
            {COMPOSITION_RINGS.map((ring) => {
              const ringScore =
                ring.id === "biomarkers"
                  ? scores.finalBiomarkersScore
                  : ring.id === "recovery"
                    ? scores.finalRecoveryScore
              ...
              return (
                <CompositionCard
                  ...
                  weight={ring.weight}
                  onPress={() =>
                    navigation.navigate("RingDetail", { id: ring.id })
                  }
                />
              );
            })}

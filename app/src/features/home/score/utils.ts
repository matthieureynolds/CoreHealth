...
export const deriveDashboardScores = (
  ...
  const biomarkers = (healthScore?.biomarkers ?? 0) > 0 ? healthScore!.biomarkers! : 77;
  const recovery   = (healthScore?.recovery   ?? 0) > 0 ? healthScore!.recovery!   : 79;
  const lifestyle  = (healthScore?.activity   ?? 0) > 0 ? healthScore!.activity!   : 72;

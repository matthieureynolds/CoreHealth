...
export function useMediaInput({
  ...
  const sendImageWithText = async () => {
    ...
    try {
      ...
      const prompt = imageInputText.trim() || 'Please analyze this image and provide any relevant health insights or descriptions.';
      const { analysis } = await DataService.analyzeImage(selectedImage.base64 ?? '', selectedImage.mimeType || 'image/jpeg', prompt);
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'assistant', content: stripEmojis(analysis || '[No analysis returned]'), timestamp: new Date() }]);
    } catch (error) {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      try {
        ...
        if (asset.mimeType && asset.mimeType.startsWith('image/')) {
          ...
          const { analysis } = await DataService.analyzeImage(base64, asset.mimeType || 'image/jpeg');
          ...
          try {
            ...
            if (fileInfo.exists && (fileInfo as any).size > MAX_PDF_BYTES) {
              aiContent = "This PDF is too large to process (over 5MB). Try splitting it into smaller files, or enter the key values manually.";
            ...
            const response = await api.post<{ reply: string; biomarkersAdded: number; recordsAdded: number }>(
              ...
              { message: '', fileBase64, fileName: asset.name },
            );
            ...
            const isTimeout = e?.message?.includes('504') || e?.message?.includes('timeout') || e?.message?.includes('408');
            aiContent = isTimeout
...
export function usePlanHistory({
  ...
  const openTimelineHistory = async (_type: 'Supplement' | 'Appointment' | 'Lab', title: string) => {
    setTimelineHistoryTitle(title); setShowTimelineHistory(true);
    setTimelineHistory([]); setTimelineHistoryLoading(false);
  };

  const handleLoadPlanHistory = async () => {
    setShowPlanHistory(true);
    setPlanHistory([]); setPlanHistoryLoading(false);
  };

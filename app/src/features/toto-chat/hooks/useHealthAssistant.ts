...
export function useHealthAssistant() {
  ...
  const [showMessagesAshAnimation, setShowMessagesAshAnimation] =
    useState(false);
  const messagesFadeAnim = useRef(new Animated.Value(1)).current;
  const messagesContainerRef = useRef<View>(null);
  const [messagesContainerLayout, setMessagesContainerLayout] = useState({
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  const {
    ...
    waveformAnimValues,
    recordingPulseAnim,
    handleVoiceInput: voiceInputHandler,
  } = useVoiceRecording();
  ...
  const [timelineHistory, setTimelineHistory] = useState<
    Array<{ id: string; occurred_at: string; title: string; meta?: any }>
  >([]);
  ...
  const mediaInput = useMediaInput({
    ...
    generateId,
    stripEmojis,
    onBiomarkersAdded: refreshAllHealthData,
  });
  const planHistoryHook = usePlanHistory({
    setPlanHistory,
    setShowPlanHistory,
    ...
    const initializeConversation = async () => {
      if (!isInitialized) {
        try {
          ...
          if (user) {
            try {
              ...
              if (backendHistory.length > 0) {
                ...
                await HealthAssistantService.saveConversationHistory(
                  history as any,
                );
              }
            } catch {
          ...
          if (history.length === 0) {
            history =
              (await HealthAssistantService.loadConversationHistory()) as ChatMessage[];
          }
          ...
          if (history.length > 0) {
            ...
            const profileForAI = profile
              ? {
                  ...profile,
                  displayName:
                    (user as any)?.displayName ?? (profile as any)?.displayName,
                  preferredName:
                    (user as any)?.preferredName ??
                    (profile as any)?.preferredName,
                  firstName:
                    (user as any)?.firstName ?? (profile as any)?.firstName,
              ...
              : user
                ...
                : null;
            const greeting =
              await HealthAssistantService.getPersonalizedGreeting(
                profileForAI as any,
                biomarkers || [],
                healthScore,
              );
  ...
  useEffect(() => {
    const keyboardDidShowListener =
      require("react-native").Keyboard.addListener(
        "keyboardDidShow",
        scrollToBottomNow,
      );
    return () => {
  ...
  const persistSession = async (msgs: ChatMessage[]) => {
    ...
    const session: ChatSession = {
      id: currentChatId,
      title:
        titleCandidate.length === 30 ? `${titleCandidate}…` : titleCandidate,
  ...
  const handleSendWithAnimation = async (text: string, _clientId: string) => {
    ...
    try {
      ...
      const response = await api.post<{
        ...
        recordsAdded: number;
      }>("/ai/chat", { message: userMessage.content });
      currentContent = response.reply || "";
      if (response.recordsAdded > 0) {
        refreshAllHealthData().catch((e) =>
          console.warn("Health data refresh failed:", e),
        );
      }
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        "Failed to get response from health assistant. Please try again.",
      );
      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantMessageId
            ? {
                ...m,
                content:
                  "Sorry, I had trouble responding. Please check your connection and try again.",
              }
            : m,
        ),
      );
  ...
  const humanizeCommand = (cmd: Command): string => {
    switch (cmd.type) {
      case "SUPPLEMENT_VITC_RECOMMEND":
        return "Recommend Vitamin C plan";
      case "APPT_RESCHEDULE_DENTIST":
        return "Reschedule dentist appointment";
      case "SYMPTOM_LOG_LEG_PAIN":
        return "Log leg pain and start plan";
      case "ALLERGY_UPDATE_PNUT":
        return "Update peanut allergy";
      case "TRAVEL_ADD_COUNTRY_CARD":
        return "Add country card";
      case "LAB_SUBMIT_RESULTS":
        return "Submit lab results";
      case "TRIP_CHANGE_DATES":
        return "Change trip dates";
    }
  };
  ...
  const logTimelineForCommand = async (cmd: Command, result: any) => {
    try {
      switch (cmd.type) {
        case "SUPPLEMENT_VITC_RECOMMEND": {
          const dose =
            result?.recommendedDoseMg ?? cmd.payload?.dosePreferenceMg ?? 500;
          await postTimeline({
            type: "Supplement",
        ...
        case "APPT_RESCHEDULE_DENTIST": {
          await postTimeline({
            ...
            meta: {
              old: result?.oldDateTime,
              new: result?.newDateTime,
        ...
        case "SYMPTOM_LOG_LEG_PAIN": {
          ...
          await postTimeline({
            type: "Symptom",
            title: `${side} ${region} pain logged`.replace(
              /^unspecified\s/i,
              "",
            ),
            meta: {
              severity: cmd.payload?.severity,
              onset: cmd.payload?.onsetDate,
              planId: result?.symptomId,
            },
          });
          break;
        }
        case "ALLERGY_UPDATE_PNUT": {
          await postTimeline({
            ...
            meta: {
              severity: cmd.payload?.severity,
              epiPen: cmd.payload?.epiPenOwned,
              status: result?.status,
            },
          });
          break;
        ...
        case "LAB_SUBMIT_RESULTS": {
          await postTimeline({
            ...
            meta: {
              specimenDate: cmd.payload?.specimenDate,
              updatedCount: result?.updatedBiomarkers?.length,
            },
          });
          break;
        }
  ...
  const confirmPendingCommand = async () => {
    ...
    try {
      ...
      if (
        ...
        try {
          ((result as any).plan.steps || []).forEach((s: any) => {
            if (s?.toggleKey) init[s.toggleKey] = false;
          });
        } catch {
      ...
      Alert.alert(
        "Action failed",
        e?.message || "Unable to complete the action.",
      );
    } finally {
  ...
  const handleVoiceInput = async () => {
    await voiceInputHandler((text: string) => setInputText(text));
  };
  ...
  return {
    ...
    showImageInputModal: mediaInput.showImageInputModal,
    setShowImageInputModal: mediaInput.setShowImageInputModal,
    selectedImage: mediaInput.selectedImage,
    imageInputText: mediaInput.imageInputText,
    setImageInputText: mediaInput.setImageInputText,
    dot1Anim,
    dot2Anim,
    dot3Anim,
    handleSendWithAnimation,
    humanizeCommand,
    openTimelineHistory: planHistoryHook.openTimelineHistory,

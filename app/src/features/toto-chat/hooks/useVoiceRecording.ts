        ...
        try {
          ...
          if (attempt === 1) {
            ...
            Alert.alert(
              ...
              "Voice input is not available right now. Please make sure the app is in the foreground and try again.",
            );
            return;
          }
          ...
          try {
            ...
            if (uri) {
              try {
                ...
                console.log(
                  `Audio file size: ${sizeKB} KB (base64 length: ${audioBase64.length})`,
                );
                const { transcript } = await DataService.transcribeAudio(
                  audioBase64,
                  "audio/m4a",
                );
                ...
                Alert.alert(
                  ...
                  `Could not transcribe audio. ${detail}`,
                );
              }
            }
          } catch (stopError) {
            ...
            try {
              ...
              waveformAnimValues.forEach((v, i) => {
                ...
                const shaped =
                  0.15 +
                  norm *
                    (0.2 + 0.65 * Math.abs(Math.sin(Date.now() / 250 + phase)));
                Animated.timing(v, {
                  toValue: shaped,
                  duration: 100,
              ...
              /* audio animation failure is non-fatal */
            }
          }, 100);
        } catch (startError) {

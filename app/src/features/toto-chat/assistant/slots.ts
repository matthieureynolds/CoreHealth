...
export interface TimelineEntry {
  id: string;
  type: 'Supplement'|'Appointment'|'Symptom'|'Allergy'|'Lab'|'Travel'|'System';
  title: string;
  description?: string;
...
export interface SupplementAdviceResult {
  ...
  notes: string[];
  products?: Array<{name:string; form:'tablet'|'capsule'|'powder'; doseMg:number; price:number; url?:string}>;
}
...
export interface SymptomPlan {
  steps: Array<{order:number; title:string; details:string; toggleKey:string}>; // toggle to mark done
...
// labs
export interface LabValue { biomarker: string; value: number; unit: string; refLow?: number; refHigh?: number }
export interface LabSubmitInput { panel: string; specimenDate: ISODate; values: LabValue[]; source?: 'pdf'|'image'|'manual'|'hl7' }
export interface LabSubmitResult { panelId: string; updatedBiomarkers: Array<{name:string; trend:'up'|'down'|'flat'; zScore?: number; status:'normal'|'high'|'low'}> }

// travel
export interface TravelChangeInput { tripId?: string; destination?: string; startDate: ISODate; endDate: ISODate; purpose?: string }
export interface TravelChangeResult { tripId: string; old: {start:ISODate; end:ISODate}; now: {start:ISODate; end:ISODate}; advisoriesRefreshed: boolean }

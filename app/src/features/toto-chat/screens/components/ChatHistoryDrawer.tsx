...
const ChatHistoryDrawer: React.FC<ChatHistoryDrawerProps> = ({
  ...
  const sortedSessions = useMemo(() =>
    (chatSessions || []).slice().sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime()),
    [chatSessions]
  );
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View
            ref={drawerRef}
            onLayout={() => { drawerRef.current?.measureInWindow((_x, y) => setDrawerPageY(y)); }}
            style={[styles.chatDrawer, { overflow: 'visible' }]}
          >
            {/* Header: Toto + X */}
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 20 }}>
              <Text style={{ color: '#FFFFFF', fontSize: 22, fontWeight: '700' }}>Toto</Text>
              <TouchableOpacity
                onPress={onClose}
                style={{
                  width: 36,
            ...
            >
              ...
              <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '500' }}>New chat</Text>
            </TouchableOpacity>
            ...
            >
              <Ionicons name="search-outline" size={20} color="#8E8E93" style={{ marginRight: 12 }} />
              {showSearch ? (
                <TextInput
                  value={searchQuery}
                  ...
                  style={{ flex: 1, color: '#FFFFFF', fontSize: 16, fontWeight: '500', paddingVertical: 0 }}
                />
              ) : (
                <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '500' }}>Search chats</Text>
              )}
            </TouchableOpacity>

            {/* Recent label */}
            <Text style={{ color: '#8E8E93', fontSize: 13, fontWeight: '600', paddingHorizontal: 20, marginTop: 16, marginBottom: 8 }}>Recent</Text>
            ...
            <View style={{ flex: 1, overflow: 'visible' }}>
              ...
              {!historyLoading && !historyError && (
                filteredSessions.length > 0 ? (
                  <ScrollView showsVerticalScrollIndicator={false} style={{ overflow: 'visible' }} contentContainerStyle={{ overflow: 'visible' }}>
                    {filteredSessions.map(session => (
                      <SessionItem
                        ...
                        onDeleted={(id) => { setAshY(null); onSessionDeleted(id); }}
                  ...
                  <View style={styles.centeredFill}>
                    <Text style={styles.mutedText}>{searchQuery ? 'No matches found.' : 'No previous chats yet.'}</Text>
                  </View>
                )
              )}
            </View>

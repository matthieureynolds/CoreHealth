  ...
  return (
    <View style={styles.container}>
      ...
      <NewChatModal
        ...
        onStartWithMemory={() => {
          dismissNewChatModal();
          setTimeout(() => startNewChat(true), 260);
        }}
        onStartWithoutMemory={() => {
          dismissNewChatModal();
          setTimeout(() => startNewChat(false), 260);
        }}
        styles={styles}
      />
      ...
      >
        {pendingCommand?.type === "APPT_RESCHEDULE_DENTIST" && (
          <DateTimeCollector
            label="Pick new date & time"
            onPicked={(iso, time) => {
              setPendingCommand({
                type: "APPT_RESCHEDULE_DENTIST",
                payload: {
                  ...(pendingCommand as any).payload,
                  newDate: iso,
                  newTime: time,
                },
              });
        ...
        {pendingCommand?.type === "SYMPTOM_LOG_LEG_PAIN" && (
          <SeverityCollector
            onPicked={(sev) => {
              setPendingCommand({
                type: "SYMPTOM_LOG_LEG_PAIN",
                payload: { ...(pendingCommand as any).payload, severity: sev },
              });
            }}
          />
        )}
        {pendingCommand?.type === "SUPPLEMENT_VITC_RECOMMEND" && (
          <MgCollector
            onPicked={(mg) => {
              setPendingCommand({
                type: "SUPPLEMENT_VITC_RECOMMEND",
                payload: {
                  ...(pendingCommand as any).payload,
                  dosePreferenceMg: mg,
                },
        ...
        >
          {!currentChatHasMemory && !messages.some((m) => m.role === "user") ? (
            ...
            >
              ...
              >
                This conversation won't appear in your chat history and won't be
                used to improve Toto's memory. Messages are not stored.
              </Text>
            </View>
          ) : (
          ...
          {showMessagesAshAnimation && (
            ...
            >
              <AshParticles
                visible={showMessagesAshAnimation}
                onComplete={() => {}}
              />
            </View>
          )}
        ...
        {pendingCommand && (
          <View style={styles.confirmBar}>
            <Text style={styles.confirmText}>
              {humanizeCommand(pendingCommand)}
            </Text>
            <View style={{ flexDirection: "row" }}>
              ...
              >
                <Text style={styles.confirmBtnText}>
                  {isDispatching ? "Working…" : "Confirm & Log"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        {pendingCommand?.type === "SUPPLEMENT_VITC_RECOMMEND" && (
          <View style={styles.historyLinkRow}>
            <TouchableOpacity
              onPress={() =>
                openTimelineHistory("Supplement", "Supplement History")
              }
            >
        ...
        {pendingCommand?.type === "APPT_RESCHEDULE_DENTIST" && (
          <View style={styles.historyLinkRow}>
            <TouchableOpacity
              onPress={() =>
                openTimelineHistory("Appointment", "Appointment History")
              }
            >
              <Text style={styles.historyLinkText}>
        ...
        {pendingCommand?.type === "LAB_SUBMIT_RESULTS" && (
          <View style={styles.historyLinkRow}>
            <TouchableOpacity
              onPress={() => openTimelineHistory("Lab", "Lab History")}
            >
          ...
          <View style={styles.planContainer}>
            <View style={styles.planHeader}>
              ...
              <Text style={styles.planCompletion}>
                ...
                {(activeSymptomPlan.steps || []).length}
              </Text>
            </View>
            <SymptomPlan
              ...
              onToggle={async (toggleKey, completed) => {
                ...
                try {
                  await togglesMark({
                    toggleKey,
                    completed,
                    symptomId: activeSymptomId || undefined,
                  });
                } catch {
                  setToggleState((prev) => ({
                    ...prev,
            ...
            >
              <TouchableOpacity onPress={handleLoadPlanHistory}>
                <Text
                  style={{ color: "#93C5FD", fontWeight: "700", fontSize: 12 }}
                >
                  View history
                </Text>
              </TouchableOpacity>

...
import {
  ...
  Circle,
  TabKey,
} from "./components/leaderboardData";
...
const CommunityLeaderboardScreen: React.FC = () => {
  ...
  useEffect(() => {
    AsyncStorage.getItem("joined_leagues")
      .then((stored) => {
        if (stored) {
          try {
            setJoinedLeagues(JSON.parse(stored));
          } catch {}
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    fetchAuthSession()
      .then((session) => {
        ...
        if (userId)
          api
            .put(`/users/${userId}/joined-leagues`, joinedLeagues)
            .catch(() => {});
      })
      .catch(() => {});
  }, [joinedLeagues]);
  ...
  const handleJoinWithCode = () => {
    ...
    const matched = circles.find(
      (c) =>
        `${c.name.replace(/\s+/g, "-").toUpperCase().slice(0, 10)}-JOIN` ===
    ...
    if (matched) {
      ...
      Alert.alert(
        "Invalid code",
        "No circle found with that invite code. Ask your friend to share theirs from the Circle Detail screen.",
      );
    }
  };
          ...
          <>
              ...
              {myCircles.length === 0 ? (
                <View style={styles.emptyRow}>
                  <Text style={styles.emptyText}>
                    No circles yet — create or join one below
                  </Text>
                </View>
              ) : (
                myCircles.map((c, i) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.row,
                      i === myCircles.length - 1 && styles.lastRow,
                    ]}
                    onPress={() =>
            ...
            <View style={styles.actionRow}>
              ...
              >
                ...
                <Text style={[styles.actionBtnText, { color: "#5856D6" }]}>
                  Join with Code
                </Text>
              </TouchableOpacity>
            </View>

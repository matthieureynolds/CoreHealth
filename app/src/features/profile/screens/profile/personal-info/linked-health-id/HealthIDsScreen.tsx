...
const HealthIDsScreen: React.FC = () => {
  ...
  const [editingHealthIdId, setEditingHealthIdId] = useState<string | null>(
    null,
  );
  ...
  const saveHealthID = () => {
    ...
    if (editingHealthIdId) {
      ...
      if (isPrimaryDraft) {
        updatedHealthIDs = updatedHealthIDs.map((hid) => ({
          ...hid,
          isPrimary: false,
        }));
      }
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: () => {
            const updatedHealthIDs =
              profile?.healthIDs?.filter((hid) => hid.id !== id) || [];
            updateProfile({ ...profile, healthIDs: updatedHealthIDs });
          },
        },
      ],
  ...
  const openHealthIdOptions = (healthID: HealthID) => {
    Alert.alert("Health ID Options", "What would you like to do?", [
      {
        text: "Edit",
        onPress: () => {
  ...
  const handleScannerSave = (record: any) => {
    ...
    const match = sourceText
      .replace(/\s+/g, " ")
      .match(/[A-Z0-9]{6,}|(\d[\s-]?){6,}/i);
    const extractedId = match
      ? match[0].replace(/[^A-Za-z0-9]/g, "")
      : "9434765911";

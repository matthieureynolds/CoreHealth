...
const DateOfBirthPickerModal: React.FC<DateOfBirthPickerModalProps> = ({
  visible, currentBirthDate, onConfirm, onClose,
}) => {
  const [tempDate, setTempDate] = useState<Date>(new Date());

  useEffect(() => {
    if (visible) setTempDate(currentBirthDate ? new Date(currentBirthDate) : new Date());
  ...
  const handleConfirm = () => {
    ...
    if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < tempDate.getDate())) age--;
    if (age >= 1 && age <= 150) {
...
const styles = StyleSheet.create({
  ...
  container: { backgroundColor: '#1a1a1a', borderRadius: 20, padding: 20, minWidth: 300, maxWidth: 350, width: '100%', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8, borderWidth: 1, borderColor: '#333' },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 },
  headerButton: { padding: 4 },
  title: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  body: { alignItems: 'center' },
  picker: { backgroundColor: '#333', color: '#fff', borderRadius: 12, padding: 10, width: '100%' },
});

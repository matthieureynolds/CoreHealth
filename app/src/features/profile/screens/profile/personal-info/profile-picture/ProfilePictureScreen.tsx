...
const ProfilePictureScreen: React.FC = () => {
  ...
  const pickFromLibrary = async () => {
    ...
    if (!permission.granted) {
      Alert.alert('Permission Required', 'Please allow access to your photo library in Settings.', [{ text: 'OK' }]);
      return;
    }
    ...
    <View style={styles.container}>
      <View style={styles.header} pointerEvents="box-none">
        ...
        <TouchableOpacity onPress={handleSave} style={styles.saveButton} disabled={!selectedImage}>
          <Text style={[styles.saveButtonText, !selectedImage && styles.saveButtonDisabled]}>Save</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.tabContent}>
        {activeTab === 'library' ? (
          ...
          <View style={styles.actionCard}>
            <Ionicons name="camera-outline" size={48} color="#34C759" style={{ marginBottom: 16 }} />
            <Text style={styles.actionTitle}>Take a Photo</Text>
            <Text style={styles.actionSubtitle}>Use your camera to take a new photo for your profile picture</Text>
            <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#34C759' }]} onPress={takePhoto} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  header: {
    paddingTop: 72, paddingBottom: 12, backgroundColor: '#181818',
    borderBottomWidth: 1, borderBottomColor: '#222', flexDirection: 'row',
    alignItems: 'flex-end', justifyContent: 'space-between', paddingHorizontal: 20,
  },

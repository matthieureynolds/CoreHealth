...
const FamilyHistoryScreen: React.FC = () => {
  ...
  const config = useMemo(
    () => ({
      ...
      updateItems: (familyHistory: FamilyCondition[]) =>
        updateProfile({ ...profile, familyHistory }),
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: 110, padding: 20 }}
      >
        <TouchableOpacity
          style={[styles.familyCard, { borderColor: "#3AABF0" }]}
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.condition}>
                Leverage family history without sharing anyone's data
              </Text>
        ...
        {familyHistory.length ? (
              ...
              <View style={styles.familyHeader}>
                <View style={styles.familyInfo}>
                  <Text style={styles.relation}>
                    {item.relation}
                    {(item as any).side
                      ? ` • ${((item as any).side as string).charAt(0).toUpperCase()}${((item as any).side as string).slice(1)}`
                      : ""}
                  </Text>
                  <Text style={styles.condition}>{item.condition}</Text>
                  {item.ageOfOnset && (
                    <Text style={styles.ageOfOnset}>
                      Age of onset: {item.ageOfOnset} years old
                    </Text>
                  )}
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              Add family medical conditions to help with risk assessment
            </Text>
            <TouchableOpacity

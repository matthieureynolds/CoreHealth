...
const FamilyHistoryFormModal: React.FC<FamilyHistoryFormModalProps> = ({
  ...
  useEffect(() => {
    if (visible) {
      if (editingFamily) {
        ...
        setAgeOfOnset(
          editingFamily.ageOfOnset
            ? new Date(
                new Date().getFullYear() - editingFamily.ageOfOnset,
                0,
                1,
              )
            : null,
        ...
        setResolvedDate(
          (editingFamily as any).resolvedDate
            ? new Date((editingFamily as any).resolvedDate)
            : null,
        );
      } else {
        setRelation("");
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      ageOfOnset: ageOfOnset
        ? Math.floor(
            (new Date().getTime() - ageOfOnset.getTime()) /
  ...
  return (
    <FormModalShell
      visible={visible}
      title={editingFamily ? "Edit Family History" : "Add Family History"}
      onClose={onClose}
      onSave={handleSave}
    >
      {/* Relation + Condition */}
      <View style={s.group}>
        ...
        >
          <Text
            style={[
              { fontSize: 16, color: "#FFFFFF", flex: 1 },
      ...
      <ChipSelector
        label="Side"
        options={["maternal", "paternal", ""] as const}
        selected={side}
        onSelect={setSide}
        labelMap={{ "": "Unknown" }}
      />

...
const PastAppointmentsScreen: React.FC = () => {
  ...
  const appointments = (profile?.pastAppointments ?? [])
    .slice()
    .sort((a, b) => {
      const dA =
  ...
  const addAppointment = () => {
    ...
    updateProfile({
      ...profile,
      pastAppointments: [...(profile?.pastAppointments ?? []), newAppointment],
    });
    ...
    Alert.alert("Delete appointment", "Remove this past appointment?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () => {
          updateProfile({
            ...profile,
            pastAppointments: (profile?.pastAppointments ?? []).filter(
              (a) => a.id !== id,
            ),
          });
  ...
  const openAttachment = (apt: PastAppointment) => {
    ...
    setViewingFileType(
      apt.fileUrl.toLowerCase().includes(".pdf") ? "application/pdf" : "",
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: 95, paddingBottom: 40 }}
      >
          ...
          {appointments.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>
                Add appointments you attended and attach notes or documents from
                the doctor
              </Text>
              <TouchableOpacity

...
const ScreeningFormModal: React.FC<ScreeningFormModalProps> = ({ visible, editingScreening, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (visible) {
      if (editingScreening) {
        ...
        setName(''); setScreeningDate(null); setResult('normal');
        setNextDueDate(null); setLocation(''); setNotes(''); setAttachments([]);
      }
    }
  }, [visible, editingScreening]);

  const handleSave = () => {
    if (!name.trim()) { Alert.alert('Error', 'Please enter a screening name'); return; }
    onSave({
  ...
  return (
    <FormModalShell visible={visible} title={editingScreening ? 'Edit Screening' : 'Add Screening'} onClose={onClose} onSave={handleSave}>
      <View style={s.group}>
        <SuggestionInput value={name} onChangeText={setName} placeholder="Screening name *" suggestions={COMMON_SCREENINGS} />
      </View>

      <DateField label="Screening Date" value={screeningDate} onChange={setScreeningDate} maximumDate={new Date()} />

      <ChipSelector label="Result" options={['normal', 'abnormal', 'inconclusive'] as const} selected={result} onSelect={setResult} />

      <DateField label="Next Due Date" value={nextDueDate} onChange={setNextDueDate} />

      <View style={[s.group, { marginTop: 20 }]}>
        <TextInput style={s.input} value={location} onChangeText={setLocation} placeholder="Location" placeholderTextColor="#555" />
        <View style={s.divider} />
        <TextInput style={[s.input, { height: 80, textAlignVertical: 'top' }]} value={notes} onChangeText={setNotes} placeholder="Notes" placeholderTextColor="#555" multiline numberOfLines={3} />
      </View>

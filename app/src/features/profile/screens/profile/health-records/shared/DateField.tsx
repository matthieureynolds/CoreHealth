...
const DateField: React.FC<DateFieldProps> = ({ label, value, onChange, maximumDate, minimumDate, defaultMode = 'full' }) => {
  ...
  return (
    <>
      ...
      <ChipSelector options={['year', 'yearMonth', 'full'] as const} selected={mode} onSelect={setMode} labelMap={DATE_MODE_LABELS} />
      <TouchableOpacity style={s.dateRow} onPress={() => setShowPicker(true)}>
        <Text style={[s.dateText, !value && s.placeholder]}>{formatDateForMode(value, mode)}</Text>
      ...
      {showPicker && (
        <IOSDatePicker
          ...
          onConfirm={(d) => { onChange(d); setShowPicker(false); }}

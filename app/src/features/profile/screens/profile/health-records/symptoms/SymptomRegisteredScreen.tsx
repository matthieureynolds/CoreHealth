...
const TABS = [
  { key: 'all'      as FilterType, label: 'All',      icon: 'apps-outline',   activeIcon: 'apps',   color: '#007AFF' },
  { key: 'physical' as FilterType, label: 'Physical', icon: 'body-outline',   activeIcon: 'body',   color: '#FF9500' },
  { key: 'mental'   as FilterType, label: 'Mental',   icon: 'flower-outline', activeIcon: 'flower', color: '#AF52DE' },
];
...
const getSeverityColor = (s: number) => s <= 3 ? '#4CD964' : s <= 5 ? '#FFD93D' : s <= 7 ? '#FF9500' : '#FF3B30';
const getSeverityLabel = (s: number) => s <= 2 ? 'Mild' : s <= 4 ? 'Moderate' : s <= 6 ? 'Notable' : s <= 8 ? 'Severe' : 'Critical';
...
const getDurationLabel = (d: string): string =>
  ({ just_started: 'Just started', '1_hour': '~1 hour', few_hours: 'Few hours', all_day: 'All day', multiple_days: 'Multiple days' })[d] ?? d;

const groupByDate = (symptoms: SymptomEntry[]): { title: string; data: SymptomEntry[] }[] => {
...
const SymptomRegisteredScreen: React.FC = () => {
  ...
  const animScales = (activeIdx: number) => {
    scales.forEach((s, i) =>
      Animated.spring(s, { toValue: i === activeIdx ? 1.12 : 1.0, useNativeDriver: true, tension: 200, friction: 14 }).start()
    );
  };
  ...
  const springToTab = (idx: number) => {
    Animated.spring(indicatorY, {
      ...
      useNativeDriver: true, tension: 160, friction: 14,
    }).start();
    animScales(idx);
  };
  ...
  const nearestIdx = (fingerY: number): number => {
    ...
    let best = 0;
    for (let i = 1; i < 3; i++) {
      if (Math.abs(fingerY - centers[i]) < Math.abs(fingerY - centers[best])) best = i;
    }
    return best;
  };
  ...
  const onTabLayout = (idx: number) => (e: any) => {
    ...
    if (measured.current >= 3) {
      indicatorY.setValue(indicatorSnapY(FILTER_ORDER.indexOf(filterRef.current)));
    }
  };
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [s, st] = await Promise.all([symptomService.getSymptoms(), symptomService.getSymptomStats()]);
      setSymptoms(s); setStats(st);
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Delete Symptom', 'Are you sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await symptomService.deleteSymptom(id); await loadData(); }
        catch { Alert.alert('Error', 'Failed to delete symptom'); }
      }},
    ]);
  };

  const filtered = filter === 'all' ? symptoms : symptoms.filter(s => s.type === filter);
  const grouped  = useMemo(() => groupByDate(filtered), [filtered]);
  const weekCount = symptoms.filter(s => new Date(s.timestamp).getTime() > Date.now() - 7 * 86400000).length;
  const activeColor = TABS[FILTER_ORDER.indexOf(filter)].color;

  if (loading) {
    return <View style={st.loading}><ActivityIndicator size="large" color="#007AFF" /></View>;
  }
  ...
  return (
    <View style={st.container}>
      ...
      <View style={st.header} pointerEvents="box-none">
        <TouchableOpacity onPress={() => navigation.goBack()} style={st.backBtn} hitSlop={{ top: 16, left: 16, right: 16, bottom: 16 }}>
          <Ionicons name="chevron-back" size={24} color="#FFF" />
        </TouchableOpacity>
        <Text style={st.headerTitle} pointerEvents="none">Symptoms Tracked</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* ── Content ── */}
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110, paddingBottom: 40 }}>
        ...
        {stats && symptoms.length > 0 && (
          <View style={st.summaryCard}>
            ...
            <View style={st.summaryItem}>
              <Text style={[st.summaryVal, { color: getSeverityColor(stats.averageSeverity) }]}>
            ...
            <View style={st.summaryDiv} />
            <View style={st.summaryItem}>
              <Text style={[st.summaryVal, { fontSize: 15 }]} numberOfLines={1}>{stats.mostCommonSymptom || '—'}</Text>
        ...
        {grouped.length === 0 ? (
          <View style={st.empty}>
            <View style={st.emptyIcon}><Ionicons name="pulse-outline" size={36} color="#8E8E93" /></View>
            <Text style={st.emptyTitle}>No symptoms logged</Text>
            <Text style={st.emptySub}>{'When you log symptoms from the home\nscreen, they\'ll appear here'}</Text>
          </View>
        ) : (
          grouped.map(group => (
            <View key={group.title}>
              ...
              {group.data.map(symptom => (
                <TouchableOpacity key={symptom.id} style={st.card} onLongPress={() => handleDelete(symptom.id)} activeOpacity={0.7}>
                  <View style={st.cardRow}>
                    <View style={[st.dot, { backgroundColor: getSeverityColor(symptom.severity) }]} />
                    <View style={{ flex: 1 }}>
                      <View style={st.cardTop}>
                        <Text style={st.cardName}>{symptom.category}</Text>
                        <Text style={[st.cardBadge, { color: getSeverityColor(symptom.severity) }]}>{getSeverityLabel(symptom.severity)}</Text>
                      </View>
                      <Text style={st.cardMeta}>
                        {formatRelativeTime(symptom.timestamp)}{symptom.duration ? ` · ${getDurationLabel(symptom.duration)}` : ''}
                      </Text>
                      {symptom.factors && symptom.factors.length > 0 && (
                        <View style={st.factors}>
                          {symptom.factors.map(f => <View key={f} style={st.factorPill}><Text style={st.factorTxt}>{f}</Text></View>)}
                        </View>
                      )}
                      {symptom.notes ? <Text style={st.notes} numberOfLines={1}>{symptom.notes}</Text> : null}
                    </View>
                  </View>
                </TouchableOpacity>
              ))}
      ...
      <View style={st.pillOuter}>
        <BlurView intensity={60} tint="dark" style={st.pillBlur} {...panResponder.panHandlers}>
          ...
          {TABS.map((tab, idx) => {
            ...
            return (
              <TouchableOpacity key={tab.key} activeOpacity={0.7} onPress={() => handleTap(idx)} onLayout={onTabLayout(idx)}>
                <Animated.View style={[st.pillTab, { transform: [{ scale: scales[idx] }] }]}>
                  <Ionicons name={(active ? tab.activeIcon : tab.icon) as any} size={22} color={active ? tab.color : '#8E8E93'} />
                  <Text style={[st.pillLbl, active && { color: tab.color }]}>{tab.label}</Text>
                </Animated.View>
              </TouchableOpacity>
            );
          })}
...
const st = StyleSheet.create({
  ...
  header:     { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingTop: 56, paddingBottom: 12, backgroundColor: '#000', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 100, elevation: 10 },
  backBtn:    { padding: 8, width: 40 },
  headerTitle:{ color: '#FFF', fontSize: 20, fontWeight: 'bold', textAlign: 'center', flex: 1 },

  summaryCard:{ flexDirection: 'row', backgroundColor: '#1C1C1E', marginHorizontal: 20, borderRadius: 16, paddingVertical: 18, paddingHorizontal: 12, marginBottom: 20 },
  summaryItem:{ flex: 1, alignItems: 'center' },
  summaryVal: { fontSize: 22, fontWeight: '700', color: '#FFF', marginBottom: 4 },
  summaryLbl: { fontSize: 12, color: '#8E8E93', fontWeight: '500' },
  summaryDiv: { width: 1, backgroundColor: '#2C2C2E', marginVertical: 2 },

  dateHeader: { fontSize: 13, fontWeight: '600', color: '#8E8E93', marginLeft: 24, marginBottom: 10, marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.5 },

  card:       { backgroundColor: '#1C1C1E', marginHorizontal: 20, marginBottom: 8, borderRadius: 12, padding: 16 },
  ...
  factorPill: { backgroundColor: '#2C2C2E', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  factorTxt:  { fontSize: 12, color: '#8E8E93', fontWeight: '500' },
  notes:      { fontSize: 13, color: '#6E6E73', fontStyle: 'italic', marginTop: 8 },

  empty:      { alignItems: 'center', paddingVertical: 80 },
  emptyIcon:  { width: 80, height: 80, borderRadius: 40, backgroundColor: '#1C1C1E', alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
  ...
  pillBlur:   { borderRadius: 28, overflow: 'hidden', paddingVertical: PILL_PAD, paddingHorizontal: 10, borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.10)' },
  indicator:  { position: 'absolute', left: 6, right: 6, top: 0, height: INDICATOR, borderRadius: INDICATOR / 2, borderWidth: 1 },
  pillTab:    { alignItems: 'center', paddingVertical: TAB_PAD, paddingHorizontal: 8 },

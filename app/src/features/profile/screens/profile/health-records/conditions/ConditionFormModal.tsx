...
const ConditionFormModal: React.FC<ConditionFormModalProps> = ({
  ...
  const [status, setStatus] = useState<"active" | "resolved" | "managed">(
    "active",
  );
  ...
  useEffect(() => {
    if (visible) {
      if (editingCondition) {
        ...
        setDiagnosedDate(
          editingCondition.diagnosedDate
            ? new Date(editingCondition.diagnosedDate)
            : new Date(),
        );
        setResolvedDate(
          editingCondition.resolvedDate
            ? new Date(editingCondition.resolvedDate)
            : null,
        );
  ...
  return (
    ...
    >
      ...
      <ChipSelector
        label="Severity"
        options={["mild", "moderate", "severe"] as const}
        selected={severity}
        onSelect={setSeverity}
      />
      <ChipSelector
        label="Status"
        options={["active", "resolved", "managed"] as const}
        selected={status}
        onSelect={(opt) => {
          setStatus(opt);
          if (opt === "resolved" && !resolvedDate) setResolvedDate(new Date());
        }}
      />

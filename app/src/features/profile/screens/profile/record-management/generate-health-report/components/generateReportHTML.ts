...
export const generateReportHTML = (
  ...
  let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>CoreHealth Health Report</title>
<style>
...
.title{font-size:28px;font-weight:bold;color:#007AFF;margin-bottom:10px;}
.subtitle{font-size:16px;color:#666;margin-bottom:5px;}
.section{margin-bottom:30px;page-break-inside:avoid;}
.section-title{font-size:20px;font-weight:bold;color:#007AFF;border-bottom:1px solid #eee;padding-bottom:10px;margin-bottom:15px;}
.item{background-color:#f8f9fa;border-left:4px solid #007AFF;padding:15px;margin-bottom:10px;border-radius:4px;}
...
<div class="header"><div class="title">CoreHealth Health Report</div><div class="subtitle">Generated on ${currentDate}</div><div class="subtitle">Patient: ${patientName}</div></div>
<div class="warning"><strong>⚠️ Confidential Medical Information</strong><br>This report contains sensitive health information. Please handle with appropriate care and only share with authorized healthcare providers.</div>`;
  ...
  if (selectedSections.includes('medical_conditions') && profile?.medicalHistory?.length) {
  ...
  if (selectedSections.includes('medications') && profile?.medications?.length) {
  ...
  if (selectedSections.includes('family_history') && profile?.familyHistory?.length) {
  ...
  if (selectedSections.includes('vaccinations') && (profile?.vaccinations?.length ?? 0) > 0) {
  ...
  if (selectedSections.includes('screenings') && (profile?.screenings?.length ?? 0) > 0) {
  ...
  if (selectedSections.includes('medical_records') && (profile?.medicalRecords?.length ?? 0) > 0) {
  ...
  html += `<div class="footer"><p>Generated by CoreHealth App</p><p>This report is for informational purposes only and should not replace professional medical advice.</p></div></body></html>`;
  return html;
};

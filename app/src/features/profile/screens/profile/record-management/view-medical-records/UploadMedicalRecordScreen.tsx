...
const UploadMedicalRecordScreen: React.FC = () => {
  ...
  const [dateMode, setDateMode] = useState<"year" | "yearMonth" | "full">(
    "full",
  );
  ...
  const addMedicalRecord = async () => {
    ...
    try {
      const fileName =
        selectedFile.name || name.trim().replace(/\s+/g, "_") + ".pdf";
      const fileType = selectedFile.type || "application/pdf";
      const reportDateStr = recordDate
        ? recordDate.toISOString().split("T")[0]
        : undefined;

      // Step 1: Get presigned S3 URL from backend
      const { labResultId, uploadUrl } =
        await DataService.requestLabResultUpload({
          fileName,
          fileType,
          labName: name.trim(),
      ...
      const poll = async (): Promise<void> => {
        ...
        try {
          const { processing_status } =
            await DataService.getLabResultStatus(labResultId);
      ...
      Alert.alert(
        "Upload failed",
        e?.message || "Could not upload the file. Please try again.",
      );
    } finally {
      setIsUploading(false);
    }
  ...
  const getTypeIcon = (type: string) =>
    recordTypes.find((t) => t.value === type)?.icon || "document-outline";
  const getTypeLabel = (type: string) =>
    recordTypes.find((t) => t.value === type)?.label || "Other";
  const getTypeColor = (type: string) =>
    recordTypes.find((t) => t.value === type)?.color || "#3AABF0";

  const formatDateForMode = (
    d: Date | null,

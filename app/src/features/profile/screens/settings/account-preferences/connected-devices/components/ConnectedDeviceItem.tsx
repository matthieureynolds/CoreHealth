...
export const renderDeviceLogo = (device: Device) => {
  if (typeof device.logo === "string") {
    return (
      <Ionicons name={device.logo as any} size={28} color={device.color} />
    );
  }
  return (
    <View
      style={[
        styles.deviceLogoContainer,
        { backgroundColor: getLogoBg(device.name) },
      ]}
    >
      <Image
        source={device.logo}
...
const ConnectedDeviceItem: React.FC<ConnectedDeviceItemProps> = ({
  device,
  onDisconnect,
}) => (

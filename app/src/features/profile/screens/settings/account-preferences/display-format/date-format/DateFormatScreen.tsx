...
const DateFormatScreen: React.FC = () => {
  ...
  const [selected, setSelected] = useState(settings.general.dateFormat);
  const [showPicker, setShowPicker] = useState(false);
  const translateY = useRef(new Animated.Value(1000)).current;

  useEffect(() => { setSelected(settings.general.dateFormat); }, [settings.general.dateFormat]);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header} pointerEvents="box-none">
        ...
        <Text style={styles.headerTitle} pointerEvents="none">Date Format</Text>
        <View style={{ width: 24 }} />
      </View>

      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110 }}>
        <View style={styles.card}>
          <Text style={styles.cardHeader}>DATE FORMAT</Text>
          <TouchableOpacity style={[styles.cardRow, styles.lastRow]} onPress={openPicker}>
            ...
            <Text style={styles.cardLabel}>Date Format</Text>
          ...
          <View style={styles.sheetContainer}>
            <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
              ...
              <View style={styles.sheetHeader}>
                ...
                <Text style={styles.sheetTitle}>Date Format</Text>
                <View style={{ width: 32 }} />
              </View>

...
const DisplayFormatScreen: React.FC = () => {
  ...
  const currentDateOption = dateOptions.find(
    (o) => o.value === settings.general.dateFormat,
  );
  const currentTimeOption = timeOptions.find(
    (o) => o.value === settings.general.timeFormat,
  );
  const currentLanguage =
    languageOptions.find(
      (o) => o.value === mapLanguageToCode(settings.general.language),
    )?.label ?? "English";
    ...
    switch (activePicker) {
      case "units":
        return {
          title: "Units",
          options: unitOptions,
          selected: settings.general.units,
      ...
      case "date":
        return {
          title: "Date Format",
          options: dateOptions,
          selected: settings.general.dateFormat,
      ...
      case "time":
        return {
          title: "Time Format",
          options: timeOptions,
          selected: settings.general.timeFormat,
      ...
      case "language":
        return {
          ...
          selected: mapLanguageToCode(settings.general.language),
  ...
  return (
    <View style={styles.container}>
      <SettingsHeader title="Display & Format" />
              ...
              >
                {pickerConfig?.options.map((option: any) => (
                  <TouchableOpacity
                    ...
                    onPress={() => pickerConfig.onSelect(option.value)}
                  >
                    <View style={{ flex: 1 }}>
                    ...
                    {pickerConfig.selected === option.value && (

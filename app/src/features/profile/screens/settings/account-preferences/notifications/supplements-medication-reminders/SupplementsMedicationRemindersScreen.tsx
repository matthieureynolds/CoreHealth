...
const SupplementsMedicationRemindersScreen: React.FC = () => {
  ...
  const [alerts, setAlerts] = useState<string[]>(['30 minutes before']);
  ...
  useEffect(() => { AsyncStorage.setItem('@notif_med_enabled', enabled ? '1' : '0'); }, [enabled]);
  useEffect(() => { AsyncStorage.setItem('@notif_med_alerts', JSON.stringify(alerts)); }, [alerts]);

  const addAlert = () => setAlerts([...alerts, '1 hour before']);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header} pointerEvents="box-none">
        ...
        <Text style={styles.headerTitle} pointerEvents="none">Medication Reminders</Text>
        <View style={{ width: 24 }} />
      </View>
        ...
        <View style={styles.card}>
          <View style={styles.toggleRow}>
            <View style={styles.labelGroup}>
              <Ionicons name="medical-outline" size={24} color="#FF9500" style={{ marginRight: 14 }} />
              <View style={{ flex: 1 }}>
                <Text style={styles.title}>Supplement & Medication Reminders</Text>
                <Text style={styles.subtitle}>Get notified when it's time to take your supplements or medications</Text>
              </View>
            </View>

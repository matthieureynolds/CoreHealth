...
const NotificationReminderScreen: React.FC<NotificationReminderScreenProps> = ({
    ...
    (async () => {
      try {
        ...
        if (al) {
          ...
          if (Array.isArray(p) && p.length) setAlerts(p);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  useEffect(() => {
    AsyncStorage.setItem(enabledStorageKey, enabled ? "1" : "0");
  }, [enabled, enabledStorageKey]);
  useEffect(() => {
    AsyncStorage.setItem(alertsStorageKey, JSON.stringify(alerts));
  ...
  const updateAlert = (i: number, value: string) => {
    const a = [...alerts];
    a[i] = value;
    setAlerts(a);
                ...
                <View style={styles.timePickerList}>
                  {NOTIFICATION_TIME_OPTIONS.map((t) => (
                    <TouchableOpacity
                      ...
                      onPress={() => updateAlert(editingIndex, t)}
                    >
                      <Text style={styles.timeOptionText}>{t}</Text>
                      {alerts[editingIndex] === t && (
                        <Ionicons name="checkmark" size={18} color="#3AABF0" />
                      )}
                    </TouchableOpacity>
                  ))}
                </View>

...
const DataSharingSettingsScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("@corehealth_location_consent")
      .then((v) => setLocationConsent(v === "true"))
      .catch(() => {});
  }, [loadHealthMemory]);

  const toggleLocationConsent = async (value: boolean) => {
    await AsyncStorage.setItem(
      "@corehealth_location_consent",
      value ? "true" : "false",
    );
    setLocationConsent(value);
    if (!value) {
      Alert.alert(
        "Location sync disabled",
        "Your location will no longer be synced to CoreHealth servers.",
      );
    }
  };
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(
                "Consent Withdrawn",
                "Your withdrawal has been recorded. Contact us at privacy@corehealth.ai to request full data erasure (GDPR Art. 17).",
              );
            } catch {
              Alert.alert(
                "Error",
                "Failed to record withdrawal. Please try again or email privacy@corehealth.ai.",
              );
            }
          },
        },
      ...
      [
        {
          ...
          onPress: () =>
            Alert.alert(
              "Settings Updated",
              "Data sharing enabled for research and app improvement.",
            ),
        },
        {
          ...
          onPress: () =>
            Alert.alert(
              "Settings Updated",
              "Only anonymized data will be shared for research purposes.",
            ),
        },
        {
          text: "No Sharing",
          onPress: () =>
            Alert.alert(
              "Settings Updated",
              "Data sharing disabled. Your data remains completely private.",
            ),
        },
        { text: "Cancel", style: "cancel" },
      ],
            ...
            {memoryLoading ? (
              ...
              <>
                ...
                {memoryUpdatedAt && (
                  <Text style={styles.memoryUpdated}>
                    Last updated:{" "}
                    {new Date(memoryUpdatedAt).toLocaleDateString()}
                  </Text>
                )}
                <View style={styles.memoryActions}>
                  <TouchableOpacity
                    style={styles.memoryViewButton}
                    onPress={() => setShowMemoryModal(true)}
                    activeOpacity={0.8}
                  >
                    <Text style={styles.memoryViewButtonText}>
                      View Full Memory
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.memoryClearButton}
                    onPress={handleClearMemory}
                    activeOpacity={0.8}
                  >
              ...
              <View style={styles.cardRow}>
                ...
                <Text style={styles.cardSub}>
                  Toto hasn't built any memory yet. Start a conversation to get
          ...
          <View style={styles.card}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardSub}>
                  GDPR Art. 7(3) — withdraw AI health data processing consent at
                  any time
                </Text>
              </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardSub}>
                  Sync your location to CoreHealth servers for travel health
                  insights
                </Text>
              </View>
              <TouchableOpacity
              ...
              >
                <View
                  style={[
                    styles.toggleThumb,
                    locationConsent && styles.toggleThumbOn,
                  ]}
                />
              </TouchableOpacity>
            </View>
            <TouchableOpacity
              style={styles.cardRow}
              onPress={handleDataSharingOptions}
            >
              <Ionicons
                name="people-outline"
                size={22}
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardSub}>
                  Control how your data is shared for research
                </Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color="#888" />
            </TouchableOpacity>

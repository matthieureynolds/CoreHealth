...
const between = (s: number, min: number, max: number) => min + (s % (max - min + 1));

type SubScore = { label: string; icon: string; color: string; score: number; change: number };
...
const CountryDetailScreen: React.FC = () => {
  ...
  const weeklyActive = Math.round(participants * (0.4 + (between(s, 0, 30) / 100)));
  ...
  const subScores: SubScore[] = useMemo(() => [
    {
      ...
      color: '#30D158',
      score: between(s ^ 1, Math.round(score * 0.85), Math.round(score * 1.08)),
    ...
    },
    {
      ...
      icon: 'sunny',
      color: '#FFD60A',
      score: between(s ^ 5, Math.round(score * 0.80), Math.round(score * 1.05)),
    ...
    },
    {
      ...
      icon: 'moon',
      color: '#BF5AF2',
      score: between(s ^ 7, Math.round(score * 0.78), Math.round(score * 1.06)),
    ...
    },
    {
      ...
      icon: 'nutrition',
      color: '#FF9F0A',
      score: between(s ^ 9, Math.round(score * 0.81), Math.round(score * 1.07)),
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
          ...
          <View style={styles.heroLeft}>
            ...
            <View>
              ...
              <View style={styles.heroMeta}>
                ...
                <Text style={styles.heroMetaText}>{participants.toLocaleString()} participants</Text>
                <Text style={styles.heroDot}>·</Text>
                <Text style={styles.heroMetaText}>{weeklyActive.toLocaleString()} active this week</Text>
              </View>
            </View>
          </View>
          <View style={styles.heroScoreBox}>
            <Text style={styles.heroScore}>{score.toFixed(1)}</Text>
            <View style={[styles.deltaBadge, delta > 0 ? styles.deltaUp : delta < 0 ? styles.deltaDown : styles.deltaFlat]}>
              {delta !== 0 && (
                <Ionicons name={delta > 0 ? 'arrow-up' : 'arrow-down'} size={10} color={delta > 0 ? '#34C759' : '#FF453A'} />
              )}
              <Text style={[styles.deltaText, { color: delta > 0 ? '#34C759' : delta < 0 ? '#FF453A' : '#8E8E93' }]}>
        ...
        <View style={styles.card}>
          {subScores.map((item, i) => (
            <View key={item.label} style={[styles.row, i === subScores.length - 1 && styles.lastRow]}>
              <View style={[styles.iconBadge, { backgroundColor: item.color + '22' }]}>
                <Ionicons name={item.icon as any} size={18} color={item.color} />
              </View>
              <Text style={styles.rowLabel}>{item.label}</Text>
              <View style={styles.barContainer}>
                <View style={[styles.barFill, { width: `${Math.min(item.score, 100)}%`, backgroundColor: item.color + 'AA' }]} />
              </View>
              <Text style={[styles.rowScore, { color: item.color }]}>{item.score}</Text>
              <View style={styles.changeChip}>
                ...
                <Text style={[styles.changeText, { color: item.change > 0 ? '#34C759' : item.change < 0 ? '#FF453A' : '#8E8E93' }]}>
            ...
            {monthlyData.map((d) => (
              <View key={d.month} style={styles.chartColumn}>
                ...
                <View style={styles.chartBarWrapper}>
                  <View style={[styles.chartBar, { height: `${(d.value / maxBar) * 100}%` }]} />
                </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            <Ionicons name="flame" size={20} color="#FF9F0A" style={{ marginBottom: 6 }} />
            <Text style={styles.statValue}>{weeklyActive.toLocaleString()}</Text>
            <Text style={styles.statLabel}>Active / week</Text>
          </View>
          <View style={styles.statCard}>
            <Ionicons name="trending-up" size={20} color="#30D158" style={{ marginBottom: 6 }} />
...
const styles = StyleSheet.create({
  ...
  heroRank: { fontSize: 11, fontWeight: '700', color: '#30D158', letterSpacing: 0.5, marginBottom: 2 },
  heroCountry: { fontSize: 20, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 4 },
  ...
  deltaBadge: {
    ...
    paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8, marginTop: 4,
  },
  ...
  barContainer: {
    flex: 1, height: 6, backgroundColor: '#2C2C2E', borderRadius: 3, overflow: 'hidden',
  },
  ...
  statCard: {
    flex: 1, backgroundColor: '#1C1C1E', borderRadius: 12, padding: 16,
    alignItems: 'center', borderWidth: 1, borderColor: '#2C2C2E',
  },
  statValue: { fontSize: 18, fontWeight: '800', color: '#FFFFFF', marginBottom: 2 },
  statLabel: { fontSize: 11, color: '#8E8E93', fontWeight: '500', textAlign: 'center' },
});

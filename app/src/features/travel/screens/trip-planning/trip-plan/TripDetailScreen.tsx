...
const TripDetailScreen: React.FC = () => {
  ...
  useEffect(() => {
    AsyncStorage.getItem(`trip_returnflight_${trip.id}`)
      .then((raw) => {
        if (raw) {
          try {
            setReturnFlight(JSON.parse(raw));
          } catch {
            /* ignore */
          }
        }
  ...
  const canSaveCommit =
    draftTitle.trim().length > 0 && isHHMM(draftStart) && isHHMM(draftEnd);
  const openAddCommit = () => {
  ...
  const saveCommitment = () => {
    ...
    const next: Commitment = {
      ...
      start_local: draftStart,
      end_local: draftEnd,
    };
    setCommitments((prev) =>
      editingCommit
        ? prev.map((x) => (x === editingCommit ? next : x))
        : [...prev, next],
    );
    setDraftTitle("");
    setDraftStart("09:00");
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top }]}>
      ...
      <Modal
        visible={showCommitForm}
        transparent
        animationType="slide"
        onRequestClose={closeCommitForm}
      >
        <View style={s.sheetRoot}>
          <TouchableOpacity
            style={s.sheetBackdrop}
            activeOpacity={1}
            onPress={closeCommitForm}
          />
          <View style={[s.sheet, { paddingBottom: insets.bottom + 20 }]}>
            <View style={s.grabber} />
            <View style={s.sheetHeader}>
              <Text style={s.sheetTitle}>
                {editingCommit ? "Edit commitment" : "Add commitment"}
              </Text>
              <TouchableOpacity
              ...
              {planDays.map((day, idx) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      s.dayChip,
                      active ? s.dayChipActive : s.dayChipIdle,
                    ]}

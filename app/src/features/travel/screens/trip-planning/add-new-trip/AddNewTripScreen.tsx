...
const AddNewTripScreen: React.FC = () => {
  ...
  const [showDatePicker, setShowDatePicker] = useState<'dep' | 'ret' | null>(null);
  ...
  const [form, setForm] = useState<TripFormData>({
    ...
    plan_style: 'gentle',
    prefs: { chronotype: 'neutral', caffeine: true, melatonin: false, naps: true },
  });
  ...
  const handleSave = () => {
    ...
    Alert.alert('Trip Created', `${form.origin} → ${form.destination} added to your trip plans.`, [
  ...
  const handleFlightLookup = async () => {
    ...
    try {
      ...
      const result = await FlightLookupService.lookupFlight(parts[1].toUpperCase(), parts[2], flightDate.trim());
      if (result) {
        setForm(f => ({ ...f, origin: result.origin_iata, destination: result.dest_iata, title: `${result.origin_iata} → ${result.dest_iata}` }));
        setActiveTab('manual');
        Alert.alert('Flight Found', `${result.origin_iata} → ${result.dest_iata} loaded.`);
      } else {
        Alert.alert('Not Found', 'Flight data unavailable. Enter manually instead.');
      }
    } catch (e: any) {
      Alert.alert('Lookup Failed', e.message ?? 'Try entering your trip manually.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110, paddingBottom: 40 }}>
        ...
        {activeTab === 'flight' ? (
          <View style={styles.section}>
            ...
            <TextInput style={styles.input} value={flightNumber} onChangeText={setFlightNumber} placeholder="BA123" placeholderTextColor="#555" autoCapitalize="characters" />
            <Text style={styles.label}>Flight Date (YYYY-MM-DD)</Text>
            <TextInput style={styles.input} value={flightDate} onChangeText={setFlightDate} placeholder="2025-12-31" placeholderTextColor="#555" />
            <TouchableOpacity style={[styles.lookupBtn, isLoading && styles.lookupBtnDisabled]} onPress={handleFlightLookup} disabled={isLoading} activeOpacity={0.8}>
              {isLoading ? <ActivityIndicator color="#fff" /> : <><Ionicons name="search-outline" size={18} color="#fff" style={{ marginRight: 8 }} /><Text style={styles.lookupBtnText}>Look Up Flight</Text></>}
            </TouchableOpacity>
          </View>
        ) : (
          <>
            <View style={styles.section}>
              ...
              <TextInput style={styles.input} value={form.title} onChangeText={v => setForm(f => ({ ...f, title: v }))} placeholder="e.g. Paris Holiday" placeholderTextColor="#555" />
              <Text style={styles.label}>Origin</Text>
              <TextInput style={styles.input} value={form.origin} onChangeText={v => setForm(f => ({ ...f, origin: v }))} placeholder="e.g. London or LHR" placeholderTextColor="#555" autoCapitalize="words" />
              <Text style={styles.label}>Destination</Text>
              <TextInput style={styles.input} value={form.destination} onChangeText={v => setForm(f => ({ ...f, destination: v }))} placeholder="e.g. Tokyo or HND" placeholderTextColor="#555" autoCapitalize="words" />
            </View>
            ...
            <View style={styles.section}>
              ...
              <TouchableOpacity style={[styles.dateRow, { borderBottomWidth: 0 }]} onPress={() => setShowDatePicker('ret')}>
                <Ionicons name="calendar-outline" size={18} color="#34C759" style={{ marginRight: 12 }} />
                <Text style={styles.dateLabel}>Return</Text>
                <Text style={styles.dateValue}>{form.returnDate.toDateString()}</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.section}>
              ...
              {(['gentle', 'aggressive'] as const).map(s => (
                <TouchableOpacity key={s} style={styles.optionRow} onPress={() => setForm(f => ({ ...f, plan_style: s }))}>
                  <View style={[styles.radio, form.plan_style === s && styles.radioSelected]} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.optionLabel}>{s === 'gentle' ? 'Gentle — gradual, low effort' : 'Aggressive — faster adjustment'}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.section}>
              ...
              {(['caffeine', 'melatonin', 'naps'] as const).map(pref => (
                <TouchableOpacity key={pref} style={styles.toggleRow} onPress={() => updatePref(pref, !form.prefs[pref])}>
                  <Text style={styles.toggleLabel}>{pref.charAt(0).toUpperCase() + pref.slice(1)}</Text>
                  <View style={[styles.togglePill, form.prefs[pref] && styles.togglePillOn]}>
                    <View style={[styles.toggleThumb, form.prefs[pref] && styles.toggleThumbOn]} />
                  </View>
                </TouchableOpacity>
              ))}
            </View>
      ...
      {showDatePicker && (
        <DateTimePicker
          value={showDatePicker === 'dep' ? form.departureDate : form.returnDate}
          mode="date"
          display={Platform.OS === 'ios' ? 'inline' : 'default'}
          minimumDate={new Date()}
          onChange={(_, date) => {
            if (date) setForm(f => showDatePicker === 'dep' ? { ...f, departureDate: date } : { ...f, returnDate: date });
...
const styles = StyleSheet.create({
  ...
  header: {
    paddingTop: 72, paddingBottom: 12, backgroundColor: '#181818', borderBottomWidth: 1,
    borderBottomColor: '#222', flexDirection: 'row', alignItems: 'flex-end',
    justifyContent: 'space-between', paddingHorizontal: 20, position: 'absolute',
    top: 0, left: 0, right: 0, zIndex: 1000, elevation: 10,
  },

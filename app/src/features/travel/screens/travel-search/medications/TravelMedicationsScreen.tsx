...
const KIT_ITEMS = [
  { name: 'Pain relievers (paracetamol / ibuprofen)', category: 'Pain & Fever' },
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header} pointerEvents="box-none">
        ...
        <Text style={styles.headerTitle} pointerEvents="none">Travel Medications</Text>
        <View style={{ width: 24 }} />
      </View>

      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110, paddingBottom: 40 }}>
        <View style={styles.progressCard}>
          <Ionicons name="bag-outline" size={32} color="#FF9500" style={{ marginBottom: 12 }} />
          <Text style={styles.progressTitle}>Medical Kit Checklist</Text>
          <Text style={styles.progressCount}>{packed} / {KIT_ITEMS.length} items packed</Text>
          <View style={styles.progressBar}>
            <View style={[styles.progressFill, { width: `${(packed / KIT_ITEMS.length) * 100}%` }]} />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {KIT_ITEMS.map(item => (
            <TouchableOpacity key={item.name} style={styles.itemRow} onPress={() => toggle(item.name)}>
              <View style={[styles.checkbox, checked[item.name] && styles.checkboxDone]}>
                {checked[item.name] && <Ionicons name="checkmark" size={14} color="#fff" />}
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.itemName, checked[item.name] && styles.itemNameDone]}>{item.name}</Text>
        ...
        <TouchableOpacity style={styles.linkBtn} onPress={() => Linking.openURL('https://wwwnc.cdc.gov/travel/page/pack-smart')}>
...
const styles = StyleSheet.create({
  ...
  progressCard: { backgroundColor: '#1C1C1E', borderRadius: 16, margin: 20, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: '#FF950030' },
  progressTitle: { fontSize: 18, fontWeight: '700', color: '#fff', marginBottom: 6 },
  progressCount: { fontSize: 14, color: '#8E8E93', marginBottom: 14 },
  progressBar: { width: '100%', height: 6, backgroundColor: '#2C2C2E', borderRadius: 3, overflow: 'hidden' },
  ...
  itemRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#2A2A2A' },

...
const ROUTINE = ['COVID-19', 'Influenza (Flu)', 'Tetanus/Diphtheria/Pertussis (Tdap)', 'Measles, Mumps, Rubella (MMR)', 'Hepatitis B', 'Pneumococcal'];
const TRAVEL_RECOMMENDED = ['Hepatitis A', 'Typhoid', 'Yellow Fever', 'Meningococcal', 'Japanese Encephalitis', 'Rabies', 'Cholera'];
const TIPS = ['Consult a travel medicine clinic 4–6 weeks before departure', 'Carry an International Certificate of Vaccination (Yellow Card)', 'Some destinations require proof of Yellow Fever vaccination for entry', 'Children and elderly may need additional doses or boosters'];

const VaccinationsScreen: React.FC = () => {
  const navigation = useNavigation();
  const [checkedVax, setCheckedVax] = useState<Record<string, boolean>>({});
  const toggle = (name: string) => setCheckedVax(prev => ({ ...prev, [name]: !prev[name] }));
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110, paddingBottom: 40 }}>
        <View style={styles.heroCard}>
          <Ionicons name="shield-checkmark-outline" size={36} color="#34C759" style={{ marginBottom: 12 }} />
          <Text style={styles.heroTitle}>Travel Vaccination Checklist</Text>
          <Text style={styles.heroDesc}>Keep track of your routine and travel vaccinations. Tap a vaccine to mark it as completed.</Text>
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>ROUTINE VACCINATIONS</Text>
          {ROUTINE.map(v => (
            <TouchableOpacity key={v} style={styles.vaccRow} onPress={() => toggle(v)}>
              <View style={[styles.checkbox, checkedVax[v] && styles.checkboxDone]}>
                {checkedVax[v] && <Ionicons name="checkmark" size={14} color="#fff" />}
              </View>
              <Text style={[styles.vaccName, checkedVax[v] && styles.vaccNameDone]}>{v}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          {TRAVEL_RECOMMENDED.map(v => (
            <TouchableOpacity key={v} style={styles.vaccRow} onPress={() => toggle(v)}>
              ...
              <Text style={[styles.vaccName, checkedVax[v] && styles.vaccNameDone]}>{v}</Text>
              <View style={styles.travelBadge}><Text style={styles.travelBadgeText}>Travel</Text></View>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          {TIPS.map((t, i) => (
            <View key={i} style={styles.tipRow}>
              <Ionicons name="information-circle-outline" size={16} color="#34C759" />
...
const styles = StyleSheet.create({
  ...
  heroTitle: { fontSize: 18, fontWeight: '700', color: '#fff', marginBottom: 8, textAlign: 'center' },
  heroDesc: { fontSize: 13, color: '#8E8E93', textAlign: 'center', lineHeight: 18 },
  ...
  vaccRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#2A2A2A' },
  checkbox: { width: 22, height: 22, borderRadius: 6, borderWidth: 2, borderColor: '#555', marginRight: 14, alignItems: 'center', justifyContent: 'center' },

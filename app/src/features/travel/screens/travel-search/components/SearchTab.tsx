...
import { View, Text, ScrollView, TouchableOpacity, Animated, RefreshControl, Keyboard, Platform, UIManager, StyleSheet, Dimensions } from 'react-native';
...
const SearchTab: React.FC<SearchTabProps> = (props) => {
  const {
    ...
    filteredCities, citySearchResults, typedCityText, travelHealth, apiErrors,
    resultsOpacity, resultsTranslateY, getRowAnim,
    onInputChange, onLocationSelect, onGetCurrentLocation, onRefresh,
    onEmergencyContactPress, onDismissSuggestions,
  ...
  useEffect(() => {
    if (!hasResults) { lastParkedFor.current = ''; return; }
  ...
  const barRevealHeight = parkOffset > 0
    ? scrollY.interpolate({
  ...
  return (
    ...
    <Animated.ScrollView
      ...
      onMomentumScrollEnd={(e) => snapSearchBar(e.nativeEvent.contentOffset.y)}
      refreshControl={
        <RefreshControl
    ...
    >
      ...
      <View ref={scrollContentRef} style={[styles.content, { paddingBottom: 120 }]}>
        {hasResults ? (
          ...
          <View style={{ marginBottom: 4, zIndex: 1002 }} onLayout={measureBar}>
            {locationBar}
          </View>
        )}
        {showInlineSuggestions && (
          <TouchableOpacity
            ...
            style={[styles.tapDismissOverlay, { height: SCREEN_HEIGHT }, hasResults && { zIndex: 99 }]}
          />
        )}
        ...
        {selectedLocation && !isLoading ? (
          <View>
            ...
            >
              {/* Result Title Row */}
              {(() => {
                const resolvedCountry = travelHealth?.country && travelHealth.country !== 'Unknown'
                  ? travelHealth.country
                  : getCountryFromCity(selectedLocation);
                const nameFromData = travelHealth?.location || selectedLocation;
                const titleText = (() => {
                  if (nameFromData && resolvedCountry && resolvedCountry !== 'Unknown') {
                    const alreadyHasCountry = nameFromData.toLowerCase().includes(resolvedCountry.toLowerCase());
                    return alreadyHasCountry ? nameFromData : `${nameFromData}, ${resolvedCountry}`;
                  }
                  return nameFromData;
                })();
                return (
                  <View style={styles.resultTitleRow}>
                    <Text style={styles.resultTitle} numberOfLines={1}>{titleText}</Text>
                    <CountryFlag country={resolvedCountry} style={{ marginLeft: 10 }} />
                  </View>
                );
              })()}
              ...
              {Object.keys(apiErrors).length > 0 && (
                <View style={bannerStyles.container}>
                  <Ionicons name="cloud-offline-outline" size={14} color="#8E8E93" />
                  <Text style={bannerStyles.text}>
                    Couldn't load {Object.keys(apiErrors).map(k => API_ERROR_LABELS[k] ?? k).join(', ')}. Pull to refresh.
                  </Text>
                </View>
              )}
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyStateText, { fontSize: 17 }]}>for comprehensive health insights</Text>
          </View>
        ) : null}
      </View>
    </Animated.ScrollView>
    ...
    {hasResults && (
      <Animated.View
        ...
        style={[
          ...
          showInlineSuggestions
            ? { zIndex: 1002 }
            : { height: barRevealHeight, overflow: 'hidden', zIndex: 100, opacity: barOpacity },
        ]}
      >
        <View onLayout={measureBar}>
          {locationBar}
        </View>
      </Animated.View>
    )}

...
// Memoised, and bails out entirely while hidden. Previously this ~35-prop tree
// was reconciled on every parent render even with the modal closed.
const AddTripModal: React.FC<AddTripModalProps> = React.memo(
  ({
    visible,
    tripModalTranslateY,
    flightCarrier,
    ...
    const canSubmit = showManualEntry
      ? newTripDepartureLocation.trim().length > 0 &&
        newTripDestination.trim().length > 0
      : flightCarrier.trim().length > 0 &&
            ...
            >
              ...
              >
                <TouchableOpacity
                  onPress={(e) => {
                    e.stopPropagation();
                    showManualEntry ? onHideManualEntry() : onClose();
                  }}
                      ...
                      initialValue={
                        ...
                        (() => {
                          if (showDatePicker === "departure")
                            return newTripDepartureDate || new Date();
                          if (showDatePicker === "return")
                            return newTripReturnDate || new Date();
                          if (showDatePicker === "departureTime")
                            return newTripDepartureTime || new Date();

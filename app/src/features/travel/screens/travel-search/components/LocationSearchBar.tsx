...
// entire travel tree ~12x a second, permanently, even while idle.
const PLACEHOLDER_CITIES = popularCities.slice(0, 8);
...
const LocationSearchBar: React.FC<LocationSearchBarProps> = ({
  ...
  const typedCityText = useTypewriter(PLACEHOLDER_CITIES);
  // Popular cities that aren't already covered by API results
  const apiCityNames = new Set(
    citySearchResults.map((c) => `${c.name}, ${c.country}`.toLowerCase()),
  );
  const extraPopularCities = filteredCities
    .filter((city) => !apiCityNames.has(city.toLowerCase()))
    .slice(0, 4);
  ...
  const progress = useRef(
    new Animated.Value(hasSuggestionContent ? 1 : 0),
  ).current;
  ...
  const cornerRadius = progress.interpolate({
    inputRange: [0, 1],
    outputRange: [12, 0],
  });
                ...
                <View>
                  ...
                  {citySearchResults.length > 0 &&
                    citySearchResults.map((city, index) => {
                      const isLast =
                        index === citySearchResults.length - 1 &&
                        extraPopularCities.length === 0;
                      return (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            !isLast ? styles.suggestionItemDivider : null,
                          ]}
                          onPress={() => {
                            onDismissSuggestions();
                  ...
                  {extraPopularCities.map((city, index) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.suggestionItem,
                        index < extraPopularCities.length - 1

...
function syncTripToBackend(trip: Trip) {
  fetchAuthSession().then(session => {
    ...
    if (!userId) {
      if (__DEV__) console.warn('[syncTrip] No user ID found in session, skipping sync');
      return;
    }
    api.post(`/users/${userId}/trips`, {
      ...
      notes: trip.notes,
      tripData: { originTimezone: trip.originTimezone, layovers: trip.layovers, jetLagPlanner: trip.jetLagPlanner, checklist: trip.checklist },
    }).catch((error) => {
...
export function validateTrip(opts: {
  ...
  if (!opts.departureLocation.trim()) return 'Please enter a departure location';
...
export interface TripHandlersParams {
  flightCarrier: string; flightNumber: string; flightLookupResult: FlightOption | null;
  flightSegments: FlightOption[]; newTripDepartureDate: Date | undefined; newTripReturnDate: Date | undefined;
  ...
  setFlightSegments: (fn: (prev: FlightOption[]) => FlightOption[]) => void; setFlightDetailsExpanded: (v: boolean) => void;
  setFlightSuggestions: (v: FlightOption[]) => void;
  setFlightLookupResult: (v: FlightOption | null) => void; setIsLookingUpFlight: (v: boolean) => void; setFlightNotFound: (v: boolean) => void;
...
export function createTripHandlers(params: TripHandlersParams) {
  ...
  const handleAddTrip = () => {
    ...
    const newTrip: Trip = {
      ...
      destination: newTripDestination.trim(), departureDate,
      returnDate: newTripReturnDate, timezone: destTz ?? 'UTC', originTimezone: originTz,
  ...
  const handleConfirmFlightTrip = () => {
    ...
    const newTrip: Trip = {
      id: Date.now().toString(), departureLocation: first.origin_city ?? first.origin_iata,
      destination: last.dest_city ?? last.dest_iata, departureDate: new Date(first.dep_local),
      returnDate: undefined, timezone: last.dest_tz, originTimezone: first.origin_tz,
  ...
  const handleSaveEditTrip = () => {
    if (!editingTrip) { Alert.alert('Error', 'No trip selected'); return; }
    const error = validateTrip({
    ...
    const editDestTz = getCityTimezone(editTripDestination) ?? editingTrip.timezone;
    const editOriginTz = getCityTimezone(editTripDepartureLocation) ?? editingTrip.originTimezone;
    const updatedTrip: Trip = {
      ...
      timezone: editDestTz, originTimezone: editOriginTz,
  ...
  return {
    ...
    handleSelectFlightSuggestion, handleAddAnotherFlight, handleEditSegment,
  };
}

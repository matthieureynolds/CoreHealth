...
function getAirQualityHealthImpacts(status: string): string[] {
  switch (status) {
    case "good":
      return ["Minimal health impacts", "Safe for most activities"];
    case "moderate":
      return [
        "Possible irritation for sensitive individuals",
...
function getPollenHealthImpacts(status: string): string[] {
  switch (status) {
    case "good":
      return ["Minimal allergy symptoms", "Most people comfortable outdoors"];
    case "moderate":
      return [
        "Allergy symptoms likely for sensitive individuals",
        "Sneezing, runny nose, itchy eyes possible",
      ];
    case "poor":
      return [
        "Significant allergy symptoms expected",
...
function getPollenRecommendations(status: string): string[] {
  switch (status) {
    ...
    case "moderate":
      return [
        "Take allergy medications before going out",
    ...
    default:
      return [
        ...
        "Take appropriate allergy precautions",
      ];
  }
}
...
function getWaterQualityExplanation(status: string): string {
  switch (status) {
    ...
    case "moderate":
      return "Water is generally safe to drink but may have taste, odor, or minor quality issues. Consider filtration for better taste.";
    case "poor":
      return "Water has elevated contaminant levels that may pose health risks. Not recommended for drinking without treatment.";
    default:
...
function getWaterQualityHealthImpacts(status: string): string[] {
  switch (status) {
    case "good":
      return ["Minimal health risks", "Safe for daily consumption"];
    case "moderate":
      return ["Possible gastrointestinal issues", "May affect taste and odor"];
    case "poor":
      return [
...
function getWaterQualityRecommendations(status: string): string[] {
  switch (status) {
    case "good":
      return ["Tap water is safe to drink", "Good for daily use"];
    case "moderate":
      return ["Use water filters for drinking", "Boil water for cooking"];
    case "poor":
      return [
...
function getUVHealthImpacts(status: string): string[] {
  switch (status) {
    ...
    case "poor":
      return [
        "Increased sunburn risk within 30–60 minutes",
  ...
  switch (status) {
    ...
    case "poor":
      return [
        ...
        "Avoid rapid ascent",
        "Consider acetazolamide if advised",
      ];
    default:
  ...
  switch (status) {
    ...
    case "moderate":
      return [
        ...
        "Use masks where advised",
      ];
    case "poor":
      return [
        "Consider postponing non-essential travel",
        "Strict hygiene and masking",
...
export function getMetricDetails(
  metricId: string,
  status: string,
): MetricDetails {
  ...
  const details: Record<string, MetricDetails> = {
    ...
    uv_index: {
      description:
        "UV Index reflects the strength of sunburn-producing ultraviolet radiation.",
      normalRange:
      ...
      riskFactors: [
        "Fair skin or photosensitive conditions",
        "Midday outdoor exposure",
        "High altitude or equatorial regions",
        "Reflective surfaces (snow/water)",
      ],
    },
    food_safety: {
      description:
        "Food safety risk reflects local hygiene, preparation practices, and contamination risk.",
      ...
      riskFactors: [
        ...
        "Poor hand hygiene",
        "Cross-contamination in street markets",
      ],
    },
    altitude: {
      description:
        "Altitude can reduce oxygen availability and affect sleep and exercise tolerance.",
      normalRange:
        "<1500m (Low) • 1500–2500m (Moderate) • 2500–3500m (High) • 3500–5500m (Very High) • >5500m (Extreme)",
    ...
    outbreaks: {
      description:
        "Summarizes notable infectious disease activity reported locally.",
      normalRange:
        "0-19 (None) • 20-39 (Low) • 40-59 (Moderate) • 60-79 (High) • 80-100 (Severe)",

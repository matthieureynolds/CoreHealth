...
const TIPS = [
  { icon: 'phone-portrait-outline' as const, tip: 'Save the local emergency number before you travel' },
  { icon: 'document-text-outline' as const, tip: 'Carry a copy of your health insurance card and travel insurance' },
  { icon: 'language-outline' as const, tip: 'Use a translation app to communicate symptoms if language is a barrier' },
  { icon: 'location-outline' as const, tip: 'Enable location services so emergency services can find you' },
];
...
const NearbyHospitalsScreen: React.FC = () => {
  ...
  const openMaps = () => {
    Alert.alert('Find Nearby Hospitals', 'Open in Maps?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Apple Maps', onPress: () => Linking.openURL('http://maps.apple.com/?q=hospital') },
      { text: 'Google Maps', onPress: () => Linking.openURL('https://www.google.com/maps/search/hospital/') },
    ]);
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 110, paddingBottom: 40 }}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>INTERNATIONAL EMERGENCY NUMBERS</Text>
          {EMERGENCY_NUMBERS.map(e => (
            <TouchableOpacity key={e.number + e.region} style={styles.emergencyRow} onPress={() => callNumber(e.number)}>
              <Ionicons name="call-outline" size={18} color="#FF3B30" style={{ marginRight: 14 }} />
              <View style={{ flex: 1 }}>
        ...
        <View style={styles.infoCard}>
          <Ionicons name="information-circle-outline" size={20} color="#8E8E93" style={{ marginRight: 12 }} />
          <Text style={styles.infoText}>Real-time nearby facilities are shown on the Travel search screen when location services are enabled.</Text>
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  emergencyRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#2A2A2A' },

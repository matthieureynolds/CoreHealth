...
const JetLagPlanningCard: React.FC<JetLagPlanningCardProps> = ({
  ...
  const formatDate = (dateString: string) => {
    ...
    return formatShortDateBySetting(
      date,
  ...
  const renderContent = (evt: JetLagPlanningEvent, onEdit?: () => void) => (
    <View>
      ...
      <View style={styles.schedulePreview}>
        ...
        {evt.sleepAdjustment.dailySchedule.map((day, idx) => {
          ...
          const thisDate = new Date(
            prepStart.getTime() + idx * 24 * 60 * 60 * 1000,
          );
          const weekdayNames = [
            "SUN",
            "MON",
            ...
            return (
              <View key={idx} style={styles.scheduleRow}>
                ...
                <Text style={styles.scheduleTimeOverlay}>
                  {formatDisplayTime(day.bedtime)} -{" "}
                  {formatDisplayTime(day.wakeTime)}
                </Text>
        ...
        {evt.sleepAdjustment.dailySchedule.length > 2 && !isExpanded && (
          ...
          >
            <Text style={styles.scheduleMore}>
              +{evt.sleepAdjustment.dailySchedule.length - 2} more days
            </Text>
          </TouchableOpacity>
        )}
        {isExpanded && (
      ...
      <View style={styles.preparationInfo}>
        ...
        <Text style={[styles.preparationText, { color: statusColor }]}>
          Start adjusting sleep schedule from{" "}
          {formatDate(evt.preparationStartDate)}
        </Text>
      </View>
    </View>
  );

...
import { LabResult } from '../../../home/recent-lab-results/results/types';
...
const getOrganSummary = (name: string, optimal: number, total: number, outOfRange: number): string => {
  const organSummaries: Record<string, { good: string; mixed: string; bad: string }> = {
    'Liver': {
      good: 'All liver enzymes in optimal range. No inflammation, no stress — your liver is performing exactly as it should.',
      mixed: 'Liver function is mostly healthy, but one or more enzymes are slightly elevated. Cut back on alcohol and processed foods.',
    ...
    'Heart / Circulatory': {
      ...
      mixed: 'Heart health is solid overall, but some lipid or inflammation markers need work. Diet and exercise will move the needle.',
    ...
    'Kidneys': {
      ...
      mixed: 'Kidney function is mostly normal, but some electrolytes are drifting. Hydration and diet adjustments can help.',
    ...
    'Lungs / Pulmonary': {
      ...
      mixed: 'Lung function is generally good, but there\'s room for improvement. Breathing exercises and cardio will help.',
    ...
    'Small Intestine': {
      ...
      mixed: 'Most nutrients look good, but you may be running low on one or two. Check the breakdown and consider supplementation.',
    ...
    'Large Intestine': {
      ...
      mixed: 'Mostly healthy, but some markers suggest mild gut imbalance. More fibre and fermented foods can help.',
    ...
    'Stomach': {
      ...
      mixed: 'Digestion is mostly fine, but minor markers worth monitoring. Watch for symptoms.',
    ...
    'Pancreas / Metabolic': {
      ...
      mixed: 'Metabolic function is okay, but early signs of insulin resistance are showing. Stay active and cut refined carbs.',
    ...
    'Thyroid': {
      ...
      mixed: 'Thyroid function is mostly normal, but one marker is drifting. Worth retesting in 3–6 months.',
    ...
    'Brain / Neurological': {
      ...
      mixed: 'Most neuro markers are fine, but cortisol suggests elevated stress. Prioritise sleep and recovery.',
  ...
  if (!summaries) {
    if (outOfRange > 0) return 'Some markers need attention. Swipe for the full breakdown and tap any biomarker for details.';
    if (optimal === total) return 'Every biomarker in optimal range. Your health indicators look excellent across the board.';
...
const SummaryPage: React.FC<{ biomarkers: PanelBiomarker[]; organName: string }> = ({ biomarkers, organName }) => {
  ...
  const outOfRange = biomarkers.filter(b => b.status !== 'optimal' && b.status !== 'normal').length;

  const rows: { label: string; count: number; color: string; icon: string }[] = [
    { label: 'Optimal', count: optimal, color: '#30D158', icon: 'checkmark-circle' },
    { label: 'Sufficient', count: normal, color: '#FF9F0A', icon: 'remove-circle' },
    { label: 'Out of Range', count: outOfRange, color: '#FF3B30', icon: 'alert-circle' },
  ];
  ...
  return (
    <View>
      <View style={s.summaryPage}>
        <SummaryRing optimal={optimal} normal={normal} outOfRange={outOfRange} total={total} />
        <View style={s.summaryBreakdown}>
          {rows.map((row, i) => (
            <View key={i} style={s.summaryRow}>
              <View style={s.summaryBadge}>
                <Ionicons name={row.icon as any} size={14} color={row.color} />
                <Text style={[s.summaryLabel, { color: row.color }]}>{row.label}</Text>
              </View>
              <Text style={[s.summaryCount, { color: '#FFFFFF' }]}>{row.count}</Text>
            </View>
          ))}
          <Text style={s.lastUpdated}>Last updated: {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
        </View>
      </View>
...
const BiomarkerListPage: React.FC<{ biomarkers: PanelBiomarker[] }> = ({ biomarkers }) => {
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              const labResult: LabResult = {
                id: biomarker.name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, ''),
                name: biomarker.name,
...
const PageDots: React.FC<{ count: number; active: number }> = ({ count, active }) => (
  <View style={s.dots}>
...
const BiomarkerDetailPanel: React.FC<BiomarkerDetailPanelProps> = ({
  ...
  if (selectedOrganData && selectedOrganData.data.name !== prevOrganRef.current) {
    ...
    >
      ...
      >
        ...
        <View style={{ width: pageWidth }}>
          <SummaryPage biomarkers={organ.data.biomarkers} organName={organ.data.name} />
        </View>

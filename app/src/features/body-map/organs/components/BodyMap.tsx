...
const LUNGS_PLACEHOLDER: Organ = {
  ...
  data: {
    name: "Lungs",
    description: "Oxygenate blood and remove CO₂.",
    biomarkers: [
  ...
  const handleHeadZoom = () => {
    ...
    Animated.parallel([
      Animated.timing(zoomScale, {
        ...config,
        toValue: zooming ? HEAD_ZOOM_SCALE : 1,
      }),
      ...
      >
        <Svg width={IMG_W} height={IMG_H}>
          {ORGAN_SHAPES.map((shape) => {
            const selected =
              selectedDataId === shape.organDataId && !shape.isHeadZone;
